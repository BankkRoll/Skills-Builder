...
- [migrate](https://docs.djangoproject.com/en/ref/django-admin/#django-admin-migrate), which is responsible for applying and unapplying
  migrations.
- [makemigrations](https://docs.djangoproject.com/en/ref/django-admin/#django-admin-makemigrations), which is responsible for creating new migrations
  based on the changes you have made to your models.
- [sqlmigrate](https://docs.djangoproject.com/en/ref/django-admin/#django-admin-sqlmigrate), which displays the SQL statements for a migration.
- [showmigrations](https://docs.djangoproject.com/en/ref/django-admin/#django-admin-showmigrations), which lists a project’s migrations and their
  status.
...
migrations on a per-app basis by modifying the [MIGRATION_MODULES](https://docs.djangoproject.com/en/ref/settings/#std-setting-MIGRATION_MODULES)
setting.
...
alteration (done via the [SchemaEditor](https://docs.djangoproject.com/en/ref/schema-editor/) class).
...
field and remove a model - and then run [makemigrations](https://docs.djangoproject.com/en/ref/django-admin/#django-admin-makemigrations):
...
one, you can use the [makemigrations--name](https://docs.djangoproject.com/en/ref/django-admin/#cmdoption-makemigrations-name) option:
...
[atomic()](https://docs.djangoproject.com/en/5.0/db/transactions/#django.db.transaction.atomic) or by passing `atomic=True` to
[RunPython](https://docs.djangoproject.com/en/ref/migration-operations/#django.db.migrations.operations.RunPython). See
[Non-atomic migrations](https://docs.djangoproject.com/en/howto/writing-migrations/#non-atomic-migrations) for more details.
...
### Swappable dependencies¶
   ...
   django.db.migrations.swappable_dependency(*value*)[¶](#django.db.migrations.django.db.migrations.swappable_dependency)
...
in [RunPython](https://docs.djangoproject.com/en/ref/migration-operations/#django.db.migrations.operations.RunPython) operations. This is done
...
If you are using the [from_queryset()](https://docs.djangoproject.com/en/5.0/db/managers/#django.db.models.from_queryset) function to
...
When the [migrate--fake-initial](https://docs.djangoproject.com/en/ref/django-admin/#cmdoption-migrate-fake-initial) option is used, these initial
...
[allow_migrate()](https://docs.djangoproject.com/en/5.0/db/multi-db/#allow_migrate) method of [database routers](https://docs.djangoproject.com/en/5.0/db/multi-db/#topics-db-multi-db-routing) to control which databases
[makemigrations](https://docs.djangoproject.com/en/ref/django-admin/#django-admin-makemigrations) checks for consistent history.

## Adding migrations to apps¶

New apps come preconfigured to accept migrations, and so you can add migrations
by running [makemigrations](https://docs.djangoproject.com/en/ref/django-admin/#django-admin-makemigrations) once you’ve made some changes.
...
will mark the migration as already applied. (Without the [migrate--fake-initial](https://docs.djangoproject.com/en/ref/django-admin/#cmdoption-migrate-fake-initial) flag, the command would error out because the tables it wants
...
Migrations can be reversed with [migrate](https://docs.djangoproject.com/en/ref/django-admin/#django-admin-migrate) by passing the number of the
...
[RunPython](https://docs.djangoproject.com/en/ref/migration-operations/#django.db.migrations.operations.RunPython) operation, or if you have
...
referencing them. Any [custom model fields](https://docs.djangoproject.com/en/howto/custom-model-fields/)
...
assist with model field deprecation using the [system checks framework](https://docs.djangoproject.com/en/5.0/checks/).
...
Django are made up of [Operations](https://docs.djangoproject.com/en/ref/migration-operations/), and
the main operation you use for data migrations is
[RunPython](https://docs.djangoproject.com/en/ref/migration-operations/#django.db.migrations.operations.RunPython).
...
[RunPython](https://docs.djangoproject.com/en/ref/migration-operations/#django.db.migrations.operations.RunPython) use it.
[RunPython](https://docs.djangoproject.com/en/ref/migration-operations/#django.db.migrations.operations.RunPython) expects a callable as its argument
which takes two arguments - the first is an [app registry](https://docs.djangoproject.com/en/ref/applications/) that has the historical versions of all your models
loaded into it to match where in your history the migration sits, and the
second is a [SchemaEditor](https://docs.djangoproject.com/en/ref/schema-editor/), which you can use to
...
[RunPython](https://docs.djangoproject.com/en/ref/migration-operations/#django.db.migrations.operations.RunPython) to run whatever logic you
...
to be able to write your own, see the [migration operations reference](https://docs.djangoproject.com/en/ref/migration-operations/) and the “how-to” on [writing migrations](https://docs.djangoproject.com/en/howto/writing-migrations/).
...
that [CreateModel](https://docs.djangoproject.com/en/ref/migration-operations/#django.db.migrations.operations.CreateModel) and
[DeleteModel](https://docs.djangoproject.com/en/ref/migration-operations/#django.db.migrations.operations.DeleteModel) cancel each other out,
and it knows that [AddField](https://docs.djangoproject.com/en/ref/migration-operations/#django.db.migrations.operations.AddField) can be
rolled into [CreateModel](https://docs.djangoproject.com/en/ref/migration-operations/#django.db.migrations.operations.CreateModel).

Once the operation sequence has been reduced as much as possible - the amount
possible depends on how closely intertwined your models are and if you have
any [RunSQL](https://docs.djangoproject.com/en/ref/migration-operations/#django.db.migrations.operations.RunSQL)
or [RunPython](https://docs.djangoproject.com/en/ref/migration-operations/#django.db.migrations.operations.RunPython) operations (which can’t
...
The command that backs all this is [squashmigrations](https://docs.djangoproject.com/en/ref/django-admin/#django-admin-squashmigrations) - pass it the
...
Use the [squashmigrations--squashed-name](https://docs.djangoproject.com/en/ref/django-admin/#cmdoption-squashmigrations-squashed-name) option if you want to set
...
see how [makemigrations](https://docs.djangoproject.com/en/ref/django-admin/#django-admin-makemigrations) deals with the problem when asked to create
brand new migrations from your models. In a future release of Django,
[squashmigrations](https://docs.djangoproject.com/en/ref/django-admin/#django-admin-squashmigrations) will be updated to attempt to resolve these errors
itself.
...
with the [migrate--prune](https://docs.djangoproject.com/en/ref/django-admin/#cmdoption-migrate-prune) option.
...
Serialization support for functions decorated with [functools.cache()](https://docs.python.org/3/library/functools.html#functools.cache)
or [functools.lru_cache()](https://docs.python.org/3/library/functools.html#functools.lru_cache) was added.
...
[for custom fields](https://docs.djangoproject.com/en/howto/custom-model-fields/#custom-field-deconstruct-method) which returns a
tuple of four items.
...
[makemigrations](https://docs.djangoproject.com/en/ref/django-admin/#django-admin-makemigrations) is run, you should also add a `__eq__()` method to
...
always run [makemigrations](https://docs.djangoproject.com/en/ref/django-admin/#django-admin-makemigrations) **with the lowest Django version you wish
to support**.
...
See also
  ...
  [The Migrations Operations Reference](https://docs.djangoproject.com/en/ref/migration-operations/)

Covers the schema operations API, special operations, and writing your
own operations.

  [The Writing Migrations “how-to”](https://docs.djangoproject.com/en/howto/writing-migrations/)
...
[Paginator](https://docs.djangoproject.com/en/ref/paginator/#django.core.paginator.Paginator) class. It does all the heavy lifting
of actually splitting a `QuerySet` into [Page](https://docs.djangoproject.com/en/ref/paginator/#django.core.paginator.Page)
objects.

## Example¶

Give [Paginator](https://docs.djangoproject.com/en/ref/paginator/#django.core.paginator.Paginator) a list of objects, plus the
...
## Paginating aListView¶

[django.views.generic.list.ListView](https://docs.djangoproject.com/en/ref/class-based-views/generic-display/#django.views.generic.list.ListView) provides a builtin way to paginate
the displayed list. You can do this by adding a
[paginate_by](https://docs.djangoproject.com/en/ref/class-based-views/mixins-multiple-object/#django.views.generic.list.MultipleObjectMixin.paginate_by) attribute to
...
## UsingPaginatorin a view function¶

Here’s an example using [Paginator](https://docs.djangoproject.com/en/ref/paginator/#django.core.paginator.Paginator) in a view
