# Database transactions¶ and more
...
[delete()](https://docs.djangoproject.com/en/5.0/topics/queries/#topics-db-queries-delete) and [update()](https://docs.djangoproject.com/en/5.0/topics/queries/#topics-db-queries-update) queries.

Django’s [TestCase](https://docs.djangoproject.com/en/5.0/testing/tools/#django.test.TestCase) class also wraps each test in a
transaction for performance reasons.

### Tying transactions to HTTP requests¶

A common way to handle transactions on the web is to wrap each request in a
transaction. Set [ATOMIC_REQUESTS](https://docs.djangoproject.com/en/ref/settings/#std-setting-DATABASE-ATOMIC_REQUESTS) to
...
When a view returns a [StreamingHttpResponse](https://docs.djangoproject.com/en/ref/request-response/#django.http.StreamingHttpResponse), reading
...
When [ATOMIC_REQUESTS](https://docs.djangoproject.com/en/ref/settings/#std-setting-DATABASE-ATOMIC_REQUESTS) is enabled, it’s
still possible to prevent views from running in a transaction.

   non_atomic_requests(*using=None*)[[source]](https://docs.djangoproject.com/en/_modules/django/db/transaction/#non_atomic_requests)[¶](#django.db.transaction.non_atomic_requests)

This decorator will negate the effect of [ATOMIC_REQUESTS](https://docs.djangoproject.com/en/ref/settings/#std-setting-DATABASE-ATOMIC_REQUESTS) for a given view:
...
Django provides a single API to control database transactions.
   ...
   atomic(*using=None*, *savepoint=True*, *durable=False*)[[source]](https://docs.djangoproject.com/en/_modules/django/db/transaction/#atomic)[¶](#django.db.transaction.atomic)
...
This is mostly a concern for [DatabaseError](https://docs.djangoproject.com/en/ref/exceptions/#django.db.DatabaseError) and its
subclasses such as [IntegrityError](https://docs.djangoproject.com/en/ref/exceptions/#django.db.IntegrityError). After such an
...
[TransactionManagementError](https://docs.djangoproject.com/en/ref/exceptions/#django.db.transaction.TransactionManagementError). You may
...
use [transaction.on_commit()](#performing-actions-after-commit)
...
is especially important if you’re using [atomic()](#django.db.transaction.atomic) in long-running
...
by setting [AUTOCOMMIT](https://docs.djangoproject.com/en/ref/settings/#std-setting-DATABASE-AUTOCOMMIT) to `False` in its
...
the open transaction is successfully committed:
   ...
   on_commit(*func*, *using=None*, *robust=False*)[[source]](https://docs.djangoproject.com/en/_modules/django/db/transaction/#on_commit)[¶](#django.db.transaction.on_commit)
...
You can use [TestCase.captureOnCommitCallbacks()](https://docs.djangoproject.com/en/5.0/testing/tools/#django.test.TestCase.captureOnCommitCallbacks) to test callbacks
...
and the [atomic()](#django.db.transaction.atomic) (or [ATOMIC_REQUESTS](https://docs.djangoproject.com/en/ref/settings/#std-setting-DATABASE-ATOMIC_REQUESTS)) transaction API. Calling [on_commit()](#django.db.transaction.on_commit) when
autocommit is disabled and you are not within an atomic block will result in an
error.

### Use in tests¶

Django’s [TestCase](https://docs.djangoproject.com/en/5.0/testing/tools/#django.test.TestCase) class wraps each test in a transaction
...
[TestCase.captureOnCommitCallbacks()](https://docs.djangoproject.com/en/5.0/testing/tools/#django.test.TestCase.captureOnCommitCallbacks). This captures your
...
[TransactionTestCase](https://docs.djangoproject.com/en/5.0/testing/tools/#django.test.TransactionTestCase) instead of
[TestCase](https://docs.djangoproject.com/en/5.0/testing/tools/#django.test.TestCase). This will mean your transactions are committed,
and the callbacks will run. However
[TransactionTestCase](https://docs.djangoproject.com/en/5.0/testing/tools/#django.test.TransactionTestCase) flushes the database between tests,
which is significantly slower than [TestCase](https://docs.djangoproject.com/en/5.0/testing/tools/#django.test.TestCase)'s isolation.
...
autocommit state of each database connection.
   ...
   get_autocommit(*using=None*)[[source]](https://docs.djangoproject.com/en/_modules/django/db/transaction/#get_autocommit)[¶](#django.db.transaction.get_autocommit)    set_autocommit(*autocommit*, *using=None*)[[source]](https://docs.djangoproject.com/en/_modules/django/db/transaction/#set_autocommit)[¶](#django.db.transaction.set_autocommit)
...
[rollback()](#django.db.transaction.rollback). These functions are defined in [django.db.transaction](#module-django.db.transaction).
   ...
   commit(*using=None*)[[source]](https://docs.djangoproject.com/en/_modules/django/db/transaction/#commit)[¶](#django.db.transaction.commit)    rollback(*using=None*)[[source]](https://docs.djangoproject.com/en/_modules/django/db/transaction/#rollback)[¶](#django.db.transaction.rollback)
...
Savepoints are controlled by three functions in [django.db.transaction](#module-django.db.transaction):
   ...
   savepoint(*using=None*)[[source]](https://docs.djangoproject.com/en/_modules/django/db/transaction/#savepoint)[¶](#django.db.transaction.savepoint)
...
known to be in a “good” state. Returns the savepoint ID (`sid`).
    ...
    savepoint_commit(*sid*, *using=None*)[[source]](https://docs.djangoproject.com/en/_modules/django/db/transaction/#savepoint_commit)[¶](#django.db.transaction.savepoint_commit)

Releases savepoint `sid`. The changes performed since the savepoint was
created become part of the transaction.

    savepoint_rollback(*sid*, *using=None*)[[source]](https://docs.djangoproject.com/en/_modules/django/db/transaction/#savepoint_rollback)[¶](#django.db.transaction.savepoint_rollback)
...
In addition, there’s a utility function:
   ...
   clean_savepoints(*using=None*)[[source]](https://docs.djangoproject.com/en/_modules/django/db/transaction/#clean_savepoints)[¶](#django.db.transaction.clean_savepoints)
...
the following functions.
   ...
   get_rollback(*using=None*)[[source]](https://docs.djangoproject.com/en/_modules/django/db/transaction/#get_rollback)[¶](#django.db.transaction.get_rollback)    set_rollback(*rollback*, *using=None*)[[source]](https://docs.djangoproject.com/en/_modules/django/db/transaction/#set_rollback)[¶](#django.db.transaction.set_rollback)
