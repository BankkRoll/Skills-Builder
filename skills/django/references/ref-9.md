...
[DJANGO_SETTINGS_MODULE](https://docs.djangoproject.com/en/topics/settings/#envvar-DJANGO_SETTINGS_MODULE) environment variable so that it points to your
...
[DJANGO_SETTINGS_MODULE](https://docs.djangoproject.com/en/topics/settings/#envvar-DJANGO_SETTINGS_MODULE) or the [--settings](#cmdoption-settings) command line
option.
...
### Getting runtime help¶
   ...
   django-admin help[¶](#django-admin-help)
...
the package containing your models. For example, if your [INSTALLED_APPS](https://docs.djangoproject.com/en/5.0/settings/#std-setting-INSTALLED_APPS)
...
### check¶
   ...
   django-admin check [app_label [app_label ...]][¶](#django-admin-check)

Uses the [system check framework](https://docs.djangoproject.com/en/5.0/checks/) to inspect the entire
...
```
    ...
    --tag TAGS, -t TAGS[¶](#cmdoption-check-tag)

The system check framework performs many different types of checks that are
[categorized with tags](https://docs.djangoproject.com/en/5.0/checks/#system-check-builtin-tags). You can use these
...
```
    ...
    --database DATABASE[¶](#cmdoption-check-database)
...
By default, these checks will not be run.
   ...
   --list-tags[¶](#cmdoption-check-list-tags)

Lists all available tags.

   --deploy[¶](#cmdoption-check-deploy)
...
module, either by setting the [DJANGO_SETTINGS_MODULE](https://docs.djangoproject.com/en/topics/settings/#envvar-DJANGO_SETTINGS_MODULE) environment
...
make it part of your integration test suite.
   ...
   --fail-level {CRITICAL,ERROR,WARNING,INFO,DEBUG}[¶](#cmdoption-check-fail-level)
...
use with the built-in gettext support. See [Internationalization and localization](https://docs.djangoproject.com/en/topics/i18n/).

   --locale LOCALE, -l LOCALE[¶](#cmdoption-compilemessages-locale)

Specifies the locale(s) to process. If not provided, all locales are processed.

   --exclude EXCLUDE, -x EXCLUDE[¶](#cmdoption-compilemessages-exclude)

Specifies the locale(s) to exclude from processing. If not provided, no locales
are excluded.

   --use-fuzzy, -f[¶](#cmdoption-compilemessages-use-fuzzy)
...
```
    ...
    --ignore PATTERN, -i PATTERN[¶](#cmdoption-compilemessages-ignore)
...
information from your settings file. See [Django’s cache framework](https://docs.djangoproject.com/en/topics/cache/) for more
information.

   --database DATABASE[¶](#cmdoption-createcachetable-database)

Specifies the database in which the cache table(s) will be created. Defaults to
`default`.

   --dry-run[¶](#cmdoption-createcachetable-dry-run)
...
[ENGINE](https://docs.djangoproject.com/en/5.0/settings/#std-setting-DATABASE-ENGINE) setting, with the connection parameters
specified in your [USER](https://docs.djangoproject.com/en/5.0/settings/#std-setting-USER), [PASSWORD](https://docs.djangoproject.com/en/5.0/settings/#std-setting-PASSWORD), etc., settings.
...
program manually.
   ...
   --database DATABASE[¶](#cmdoption-dbshell-database)

Specifies the database onto which to open a shell. Defaults to `default`.

   -- ARGUMENTS[¶](#cmdoption-dbshell-0)
...
Be aware that not all options set in the [OPTIONS](https://docs.djangoproject.com/en/5.0/settings/#std-setting-OPTIONS) part of your
database configuration in [DATABASES](https://docs.djangoproject.com/en/5.0/settings/#std-setting-DATABASES) are passed to the
...
example, the default settings don’t define [ROOT_URLCONF](https://docs.djangoproject.com/en/5.0/settings/#std-setting-ROOT_URLCONF), so
[ROOT_URLCONF](https://docs.djangoproject.com/en/5.0/settings/#std-setting-ROOT_URLCONF) is followed by `"###"` in the output of
`diffsettings`.

   --all[¶](#cmdoption-diffsettings-all)

Displays all settings, even if they have Django’s default value. Such settings
are prefixed by `"###"`.

   --default MODULE[¶](#cmdoption-diffsettings-default)

The settings module to compare the current settings against. Leave empty to
compare against Django’s default settings.

   --output {hash,unified}[¶](#cmdoption-diffsettings-output)
...
### dumpdata¶
   ...
   django-admin dumpdata [app_label[.ModelName] [app_label[.ModelName] ...]][¶](#django-admin-dumpdata)
...
[fixture](https://docs.djangoproject.com/en/topics/db/fixtures/#fixtures-explanation) for
[tests](https://docs.djangoproject.com/en/topics/testing/tools/#topics-testing-fixtures) or as an
[initial data](https://docs.djangoproject.com/en/howto/initial-data/#initial-data-via-fixtures).

Note that `dumpdata` uses the default manager on the model for selecting the
records to dump. If you’re using a [custom manager](https://docs.djangoproject.com/en/topics/db/managers/#custom-managers) as
the default manager and it filters some of the available records, not all of the
objects will be dumped.

   --all, -a[¶](#cmdoption-dumpdata-all)

Uses Django’s base manager, dumping records which might otherwise be filtered
or modified by a custom manager.

   --format FORMAT[¶](#cmdoption-dumpdata-format)

Specifies the serialization format of the output. Defaults to JSON. Supported
formats are listed in [Serialization formats](https://docs.djangoproject.com/en/topics/serialization/#serialization-formats).

   --indent INDENT[¶](#cmdoption-dumpdata-indent)

Specifies the number of indentation spaces to use in the output. Defaults to
`None` which displays all data on single line.

   --exclude EXCLUDE, -e EXCLUDE[¶](#cmdoption-dumpdata-exclude)
...
```
    ...
    --database DATABASE[¶](#cmdoption-dumpdata-database)

Specifies the database from which data will be dumped. Defaults to `default`.

   --natural-foreign[¶](#cmdoption-dumpdata-natural-foreign)
...
flag. See the [natural keys](https://docs.djangoproject.com/en/topics/serialization/#topics-serialization-natural-keys)
documentation for more details on this and the next option.

   --natural-primary[¶](#cmdoption-dumpdata-natural-primary)

Omits the primary key in the serialized data of this object since it can be
calculated during deserialization.

   --pks PRIMARY_KEYS[¶](#cmdoption-dumpdata-pks)
...
the model are output.
   ...
   --output OUTPUT, -o OUTPUT[¶](#cmdoption-dumpdata-output)
...
should drop and recreate the database and then run [migrate](#django-admin-migrate) instead.
   ...
   --noinput, --no-input[¶](#cmdoption-flush-noinput)

Suppresses all user prompts.

   --database DATABASE[¶](#cmdoption-flush-database)

Specifies the database to flush. Defaults to `default`.

### inspectdb¶

   django-admin inspectdb [table [table ...]][¶](#django-admin-inspectdb)

Introspects the database tables in the database pointed-to by the
[NAME](https://docs.djangoproject.com/en/5.0/settings/#std-setting-NAME) setting and outputs a Django model module (a `models.py`
...
- If `inspectdb` cannot map a column’s type to a model field type, it’ll
  ...
  [INSTALLED_APPS](https://docs.djangoproject.com/en/5.0/settings/#std-setting-INSTALLED_APPS). For example, [django.contrib.postgres](https://docs.djangoproject.com/en/5.0/contrib/postgres/#module-django.contrib.postgres) adds
...
[default](https://docs.djangoproject.com/en/5.0/models/fields/#django.db.models.Field.default) is specified on a model field.
...
[managed](https://docs.djangoproject.com/en/5.0/models/options/#django.db.models.Options.managed) option to `True` (or remove
...
- Models are created for partition tables if
  [--include-partitions](#cmdoption-inspectdb-include-partitions) is used.
   ...
   --database DATABASE[¶](#cmdoption-inspectdb-database)

Specifies the database to introspect. Defaults to `default`.

   --include-partitions[¶](#cmdoption-inspectdb-include-partitions)

If this option is provided, models are also created for partitions.

Only support for PostgreSQL is implemented.

   --include-views[¶](#cmdoption-inspectdb-include-views)

If this option is provided, models are also created for database views.

### loaddata¶

   django-admin loaddata fixture [fixture ...][¶](#django-admin-loaddata)

Searches for and loads the contents of the named
[fixture](https://docs.djangoproject.com/en/topics/db/fixtures/#fixtures-explanation) into the database.

   --database DATABASE[¶](#cmdoption-loaddata-database)

Specifies the database into which the data will be loaded. Defaults to
`default`.

   --ignorenonexistent, -i[¶](#cmdoption-loaddata-ignorenonexistent)

Ignores fields and models that may have been removed since the fixture was
originally generated.

   --app APP_LABEL[¶](#cmdoption-loaddata-app)

Specifies a single app to look for fixtures in rather than looking in all apps.

   --format FORMAT[¶](#cmdoption-loaddata-format)

Specifies the [serialization format](https://docs.djangoproject.com/en/topics/serialization/#serialization-formats) (e.g.,
`json` or `xml`) for fixtures [read from stdin](#loading-fixtures-stdin).

   --exclude EXCLUDE, -e EXCLUDE[¶](#cmdoption-loaddata-exclude)
...
is required to specify the [serialization format](https://docs.djangoproject.com/en/topics/serialization/#serialization-formats)
...
For more detail about fixtures see the [Fixtures](https://docs.djangoproject.com/en/topics/db/fixtures/#fixtures-explanation) topic.
...
the [i18n documentation](https://docs.djangoproject.com/en/topics/i18n/translation/#how-to-create-language-files) for details.

This command doesn’t require configured settings. However, when settings aren’t
configured, the command can’t ignore the [MEDIA_ROOT](https://docs.djangoproject.com/en/5.0/settings/#std-setting-MEDIA_ROOT) and
[STATIC_ROOT](https://docs.djangoproject.com/en/5.0/settings/#std-setting-STATIC_ROOT) directories or include [LOCALE_PATHS](https://docs.djangoproject.com/en/5.0/settings/#std-setting-LOCALE_PATHS).

   --all, -a[¶](#cmdoption-makemessages-all)

Updates the message files for all available languages.

   --extension EXTENSIONS, -e EXTENSIONS[¶](#cmdoption-makemessages-extension)
...
```
    ...
    --locale LOCALE, -l LOCALE[¶](#cmdoption-makemessages-locale)

Specifies the locale(s) to process.

   --exclude EXCLUDE, -x EXCLUDE[¶](#cmdoption-makemessages-exclude)
...
```
    ...
    --domain DOMAIN, -d DOMAIN[¶](#cmdoption-makemessages-domain)
...
- `djangojs` for `*.js` files
   ...
   --symlinks, -s[¶](#cmdoption-makemessages-symlinks)
...
```
    ...
    --ignore PATTERN, -i PATTERN[¶](#cmdoption-makemessages-ignore)
...
```
    ...
    --no-default-ignore[¶](#cmdoption-makemessages-no-default-ignore)

Disables the default values of `--ignore`.

   --no-wrap[¶](#cmdoption-makemessages-no-wrap)

Disables breaking long message lines into several lines in language files.

   --no-location[¶](#cmdoption-makemessages-no-location)
...
understand each message’s context.
   ...
   --add-location [{full,file,never}][¶](#cmdoption-makemessages-add-location)
...
Requires `gettext` 0.19 or newer.
   ...
   --no-obsolete[¶](#cmdoption-makemessages-no-obsolete)

Removes obsolete message strings from the `.po` files.

   --keep-pot[¶](#cmdoption-makemessages-keep-pot)
...
See [Customizing the makemessages command](https://docs.djangoproject.com/en/topics/i18n/translation/#customizing-makemessages) for instructions on how to customize
the keywords that [makemessages](#django-admin-makemessages) passes to `xgettext`.

### makemigrations¶

   django-admin makemigrations [app_label [app_label ...]][¶](#django-admin-makemigrations)

Creates new migrations based on the changes detected to your models.
Migrations, their relationship with apps and more are covered in depth in
[the migrations documentation](https://docs.djangoproject.com/en/topics/migrations/).
...
`makemigrations` with the app’s `app_label`.
   ...
   --noinput, --no-input[¶](#cmdoption-makemigrations-noinput)

Suppresses all user prompts. If a suppressed prompt cannot be resolved
automatically, the command will exit with error code 3.

   --empty[¶](#cmdoption-makemigrations-empty)
...
migrations.
   ...
   --dry-run[¶](#cmdoption-makemigrations-dry-run)
...
the complete migrations files that would be written.
   ...
   --merge[¶](#cmdoption-makemigrations-merge)

Enables fixing of migration conflicts.

   --name NAME, -n NAME[¶](#cmdoption-makemigrations-name)
...
name must be a valid Python [identifier](https://docs.python.org/3/reference/lexical_analysis.html#identifiers).
   ...
   --no-header[¶](#cmdoption-makemigrations-no-header)

Generate migration files without Django version and timestamp header.

   --check[¶](#cmdoption-makemigrations-check)
...
### migrate¶
   ...
   django-admin migrate [app_label] [migration_name][¶](#django-admin-migrate)
...
which migrations will be unapplied.
    ...
    --database DATABASE[¶](#cmdoption-migrate-database)

Specifies the database to migrate. Defaults to `default`.

   --fake[¶](#cmdoption-migrate-fake)
...
run correctly.
   ...
   --fake-initial[¶](#cmdoption-migrate-fake-initial)

Allows Django to skip an app’s initial migration if all database tables with
the names of all models created by all
[CreateModel](https://docs.djangoproject.com/en/5.0/migration-operations/#django.db.migrations.operations.CreateModel) operations in that
...
See [Squashing migrations](https://docs.djangoproject.com/en/topics/migrations/#migration-squashing) for more details.

### optimizemigration¶

   django-admin optimizemigration app_label migration_name[¶](#django-admin-optimizemigration)
...
to replace the named migration.
   ...
   --check[¶](#cmdoption-optimizemigration-check)
...
### runserver¶
   ...
   django-admin runserver [addrport][¶](#django-admin-runserver)
...
[WSGI_APPLICATION](https://docs.djangoproject.com/en/5.0/settings/#std-setting-WSGI_APPLICATION) setting.
...
Watchman timeout
   ...
   DJANGO_WATCHMAN_TIMEOUT[¶](#envvar-DJANGO_WATCHMAN_TIMEOUT)
...
with its own [runserver](https://docs.djangoproject.com/en/5.0/contrib/staticfiles/#staticfiles-runserver) command.

Logging of each request and response of the server is sent to the
[django.server](https://docs.djangoproject.com/en/5.0/logging/#django-server-logger) logger.

   --noreload[¶](#cmdoption-runserver-noreload)
...
have already been loaded into memory.
   ...
   --nothreading[¶](#cmdoption-runserver-nothreading)

Disables use of threading in the development server. The server is
multithreaded by default.

   --ipv6, -6[¶](#cmdoption-runserver-ipv6)
...
(such as CSS files, images, things under [MEDIA_URL](https://docs.djangoproject.com/en/5.0/settings/#std-setting-MEDIA_URL) and so forth). If
you want to configure Django to serve static media, read
[How to manage static files (e.g. images, JavaScript, CSS)](https://docs.djangoproject.com/en/howto/static-files/).

#### Serving with ASGI in development¶

Django’s `runserver` command provides a WSGI server. In order to run under
ASGI you will need to use an [ASGI server](https://docs.djangoproject.com/en/howto/deployment/asgi/).
The Django Daphne project provides [Integration with runserver](https://docs.djangoproject.com/en/howto/deployment/asgi/daphne/#daphne-runserver) that you can use.

### sendtestemail¶

   django-admin sendtestemail [email [email ...]][¶](#django-admin-sendtestemail)
...
together:
   ...
   --managers[¶](#cmdoption-sendtestemail-managers)

Mails the email addresses specified in [MANAGERS](https://docs.djangoproject.com/en/5.0/settings/#std-setting-MANAGERS) using
[mail_managers()](https://docs.djangoproject.com/en/topics/email/#django.core.mail.mail_managers).

   --admins[¶](#cmdoption-sendtestemail-admins)

Mails the email addresses specified in [ADMINS](https://docs.djangoproject.com/en/5.0/settings/#std-setting-ADMINS) using
[mail_admins()](https://docs.djangoproject.com/en/topics/email/#django.core.mail.mail_admins).
...
Starts the Python interactive interpreter.
   ...
   --interface {ipython,bpython,python}, -i {ipython,bpython,python}[¶](#cmdoption-shell-interface)
...
```
    ...
    --nostartup[¶](#cmdoption-shell-nostartup)
...
variable or the `~/.pythonrc.py` script is read.
   ...
   --command COMMAND, -c COMMAND[¶](#cmdoption-shell-command)
...
### showmigrations¶
   ...
   django-admin showmigrations [app_label [app_label ...]][¶](#django-admin-showmigrations)

Shows all migrations in a project. You can choose from one of two formats:

   --list, -l[¶](#cmdoption-showmigrations-list)
...
This is the default output format.
   ...
   --plan, -p[¶](#cmdoption-showmigrations-plan)
...
apps may also be included.
   ...
   --database DATABASE[¶](#cmdoption-showmigrations-database)

Specifies the database to examine. Defaults to `default`.

### sqlflush¶

   django-admin sqlflush[¶](#django-admin-sqlflush)

Prints the SQL statements that would be executed for the [flush](#django-admin-flush)
command.

   --database DATABASE[¶](#cmdoption-sqlflush-database)

Specifies the database for which to print the SQL. Defaults to `default`.

### sqlmigrate¶

   django-admin sqlmigrate app_label migration_name[¶](#django-admin-sqlmigrate)
...
Note that `sqlmigrate` doesn’t colorize its output.
   ...
   --backwards[¶](#cmdoption-sqlmigrate-backwards)
...
for running the migration in the forwards direction.
   ...
   --database DATABASE[¶](#cmdoption-sqlmigrate-database)

Specifies the database for which to generate the SQL. Defaults to `default`.

### sqlsequencereset¶

   django-admin sqlsequencereset app_label [app_label ...][¶](#django-admin-sqlsequencereset)
...
of sync with its automatically incremented field data.
   ...
   --database DATABASE[¶](#cmdoption-sqlsequencereset-database)

Specifies the database for which to print the SQL. Defaults to `default`.

### squashmigrations¶

   django-admin squashmigrations app_label [start_migration_name] migration_name[¶](#django-admin-squashmigrations)

Squashes the migrations for `app_label` up to and including `migration_name`
down into fewer migrations, if possible. The resulting squashed migrations
can live alongside the unsquashed ones safely. For more information,
please read [Squashing migrations](https://docs.djangoproject.com/en/topics/migrations/#migration-squashing).

When `start_migration_name` is given, Django will only include migrations
starting from and including this migration. This helps to mitigate the
squashing limitation of [RunPython](https://docs.djangoproject.com/en/5.0/migration-operations/#django.db.migrations.operations.RunPython) and
[django.db.migrations.operations.RunSQL](https://docs.djangoproject.com/en/5.0/migration-operations/#django.db.migrations.operations.RunSQL) migration operations.

   --no-optimize[¶](#cmdoption-squashmigrations-no-optimize)
...
behavior, as optimization is meant to be safe.
   ...
   --noinput, --no-input[¶](#cmdoption-squashmigrations-noinput)

Suppresses all user prompts.

   --squashed-name SQUASHED_NAME[¶](#cmdoption-squashmigrations-squashed-name)

Sets the name of the squashed migration. When omitted, the name is based on the
first and last migration, with `_squashed_` in between.

   --no-header[¶](#cmdoption-squashmigrations-no-header)

Generate squashed migration file without Django version and timestamp header.

### startapp¶

   django-admin startapp name [directory][¶](#django-admin-startapp)
...
```
    ...
    --template TEMPLATE[¶](#cmdoption-startapp-template)
...
empty list.
   ...
   --exclude DIRECTORIES, -x DIRECTORIES[¶](#cmdoption-startapp-exclude)
...
The [templatecontext](https://docs.djangoproject.com/en/5.0/templates/api/#django.template.Context) used for all matching
files is:
...
To work around this problem, you can use the [templatetag](https://docs.djangoproject.com/en/5.0/templates/builtins/#std-templatetag-templatetag)
...
### startproject¶
   ...
   django-admin startproject name [directory][¶](#django-admin-startproject)
...
```
    ...
    --template TEMPLATE[¶](#cmdoption-startproject-template)
...
The [templatecontext](https://docs.djangoproject.com/en/5.0/templates/api/#django.template.Context) used is:

- Any option passed to the `startproject` command (among the command’s
  supported options)
- `project_name` – the project name as passed to the command
- `project_directory` – the full path of the newly created project
- `secret_key` – a random key for the [SECRET_KEY](https://docs.djangoproject.com/en/5.0/settings/#std-setting-SECRET_KEY) setting
- `docs_version` – the version of the documentation: `'dev'` or `'1.x'`
- `django_version` – the version of Django, e.g. `'2.0.3'`
...
### test¶
   ...
   django-admin test [test_label [test_label ...]][¶](#django-admin-test)

Runs tests for all installed apps. See [Testing in Django](https://docs.djangoproject.com/en/topics/testing/) for more
information.

   --failfast[¶](#cmdoption-test-failfast)

Stops running tests and reports the failure immediately after a test fails.

   --testrunner TESTRUNNER[¶](#cmdoption-test-testrunner)

Controls the test runner class that is used to execute tests. This value
overrides the value provided by the [TEST_RUNNER](https://docs.djangoproject.com/en/5.0/settings/#std-setting-TEST_RUNNER) setting.

   --noinput, --no-input[¶](#cmdoption-test-noinput)
...
[DiscoverRunner](https://docs.djangoproject.com/en/topics/testing/advanced/#django.test.runner.DiscoverRunner).

   --keepdb[¶](#cmdoption-test-keepdb)
...
subsequent run. Unless the [MIGRATE](https://docs.djangoproject.com/en/5.0/settings/#std-setting-TEST_MIGRATE) test setting is
`False`, any unapplied migrations will also be applied to the test database
before running the test suite.

   --shuffle [SEED][¶](#cmdoption-test-shuffle)
...
[guarantees on test order](https://docs.djangoproject.com/en/topics/testing/overview/#order-of-tests). They also keep tests grouped
by test case class.
...
order of the original tests will be the same in the new order.
   ...
   --reverse, -r[¶](#cmdoption-test-reverse)
...
class](https://docs.djangoproject.com/en/topics/testing/overview/#order-of-tests) is preserved when using this option. This can be used
in conjunction with `--shuffle` to reverse the order for a particular seed.

   --debug-mode[¶](#cmdoption-test-debug-mode)

Sets the [DEBUG](https://docs.djangoproject.com/en/5.0/settings/#std-setting-DEBUG) setting to `True` prior to running tests. This may
help troubleshoot test failures.

   --debug-sql, -d[¶](#cmdoption-test-debug-sql)

Enables [SQL logging](https://docs.djangoproject.com/en/5.0/logging/#django-db-logger) for failing tests. If
`--verbosity` is `2`, then queries in passing tests are also output.

   --parallel [N][¶](#cmdoption-test-parallel)    DJANGO_TEST_PROCESSES[¶](#envvar-DJANGO_TEST_PROCESSES)
...
classes sequentially](https://docs.djangoproject.com/en/topics/testing/advanced/#topics-testing-enforce-run-sequentially).
...
[What can be pickled and unpickled?](https://docs.python.org/3/library/pickle.html#pickle-picklable) for details.
    ...
    --tag TAGS[¶](#cmdoption-test-tag)

Runs only tests [marked with the specified tags](https://docs.djangoproject.com/en/topics/testing/tools/#topics-tagging-tests).
...
Tests that fail to load are always considered matching.
   ...
   --exclude-tag EXCLUDE_TAGS[¶](#cmdoption-test-exclude-tag)

Excludes tests [marked with the specified tags](https://docs.djangoproject.com/en/topics/testing/tools/#topics-tagging-tests).
May be specified multiple times and combined with [test--tag](#cmdoption-test-tag).

   -k TEST_NAME_PATTERNS[¶](#cmdoption-test-k)

Runs test methods and classes matching test name patterns, in the same way as
[unittest's-koption](https://docs.python.org/3/library/unittest.html#cmdoption-unittest-k). Can be specified multiple times.

   --pdb[¶](#cmdoption-test-pdb)

Spawns a `pdb` debugger at each test error or failure. If you have it
installed, `ipdb` is used instead.

   --buffer, -b[¶](#cmdoption-test-buffer)

Discards output (`stdout` and `stderr`) for passing tests, in the same way
as [unittest's--bufferoption](https://docs.python.org/3/library/unittest.html#cmdoption-unittest-b).

   --no-faulthandler[¶](#cmdoption-test-no-faulthandler)
...
`--no-faulthandler` to disable this behavior.
   ...
   --timing[¶](#cmdoption-test-timing)

Outputs timings, including database setup and total run time.

   --durations N[¶](#cmdoption-test-durations)   New in Django 5.0.
...
### testserver¶
   ...
   django-admin testserver [fixture [fixture ...]][¶](#django-admin-testserver)
...
1. Create a test database, as described in [The test database](https://docs.djangoproject.com/en/topics/testing/overview/#the-test-database).
...
- When you’re writing [unit tests](https://docs.djangoproject.com/en/topics/testing/overview/) of how your views
  act with certain fixture data, you can use `testserver` to interact with
  the views in a web browser, manually.
- Let’s say you’re developing your Django application and have a “pristine”
  copy of a database that you’d like to interact with. You can dump your
  database to a [fixture](https://docs.djangoproject.com/en/topics/db/fixtures/#fixtures-explanation) (using the
...
templates.
   ...
   --addrport ADDRPORT[¶](#cmdoption-testserver-addrport)
...
```
    ...
    --noinput, --no-input[¶](#cmdoption-testserver-noinput)
...
[implements](https://docs.djangoproject.com/en/howto/custom-management-commands/) them has been
[enabled](https://docs.djangoproject.com/en/5.0/settings/#std-setting-INSTALLED_APPS). This section describes them grouped by
their application.
...
#### changepassword¶
   ...
   django-admin changepassword [<username>][¶](#django-admin-changepassword)

This command is only available if Django’s [authentication system](https://docs.djangoproject.com/en/topics/auth/) (`django.contrib.auth`) is installed.
...
the password whose username matches the current user.
   ...
   --database DATABASE[¶](#cmdoption-changepassword-database)
...
#### createsuperuser¶
   ...
   django-admin createsuperuser[¶](#django-admin-createsuperuser)    DJANGO_SUPERUSER_PASSWORD[¶](#envvar-DJANGO_SUPERUSER_PASSWORD)
...
[USERNAME_FIELD](https://docs.djangoproject.com/en/topics/auth/customizing/#django.contrib.auth.models.CustomUser.USERNAME_FIELD) and required
fields (listed in
[REQUIRED_FIELDS](https://docs.djangoproject.com/en/topics/auth/customizing/#django.contrib.auth.models.CustomUser.REQUIRED_FIELDS)) fall back to
...
field, you can use `DJANGO_SUPERUSER_EMAIL` environment variable.
   ...
   --noinput, --no-input[¶](#cmdoption-createsuperuser-noinput)

Suppresses all user prompts. If a suppressed prompt cannot be resolved
automatically, the command will exit with error code 1.

   --username USERNAME[¶](#cmdoption-createsuperuser-username)    --email EMAIL[¶](#cmdoption-createsuperuser-email)
...
it when running interactively.
   ...
   --database DATABASE[¶](#cmdoption-createsuperuser-database)
...
[REQUIRED_FIELDS](https://docs.djangoproject.com/en/topics/auth/customizing/#django.contrib.auth.models.CustomUser.REQUIRED_FIELDS) and want to
...
This command is only available if Django’s [contenttypes app](https://docs.djangoproject.com/en/5.0/contrib/contenttypes/) ([django.contrib.contenttypes](https://docs.djangoproject.com/en/5.0/contrib/contenttypes/#module-django.contrib.contenttypes)) is installed.
...
the deletion.
   ...
   --database DATABASE[¶](#cmdoption-remove_stale_contenttypes-database)

Specifies the database to use. Defaults to `default`.

   --include-stale-apps[¶](#cmdoption-remove_stale_contenttypes-include-stale-apps)

Deletes stale content types including ones from previously installed apps that
have been removed from [INSTALLED_APPS](https://docs.djangoproject.com/en/5.0/settings/#std-setting-INSTALLED_APPS). Defaults to `False`.

### django.contrib.gis¶

#### ogrinspect¶

This command is only available if [GeoDjango](https://docs.djangoproject.com/en/5.0/contrib/gis/)
(`django.contrib.gis`) is installed.

Please refer to its [description](https://docs.djangoproject.com/en/5.0/contrib/gis/commands/#django-admin-ogrinspect) in the GeoDjango
documentation.
...
This command is only available if the [static files application](https://docs.djangoproject.com/en/howto/static-files/) (`django.contrib.staticfiles`) is installed.

Please refer to its [description](https://docs.djangoproject.com/en/5.0/contrib/staticfiles/#django-admin-collectstatic) in the
[staticfiles](https://docs.djangoproject.com/en/5.0/contrib/staticfiles/) documentation.

#### findstatic¶

This command is only available if the [static files application](https://docs.djangoproject.com/en/howto/static-files/) (`django.contrib.staticfiles`) is installed.

Please refer to its [description](https://docs.djangoproject.com/en/5.0/contrib/staticfiles/#django-admin-findstatic) in the [staticfiles](https://docs.djangoproject.com/en/5.0/contrib/staticfiles/) documentation.
...
allows for the following options by default:
   ...
   --pythonpath PYTHONPATH[¶](#cmdoption-pythonpath)
...
```
    ...
    --settings SETTINGS[¶](#cmdoption-settings)

Specifies the settings module to use. The settings module should be in Python
package syntax, e.g. `mysite.settings`. If this isn’t provided,
`django-admin` will use the [DJANGO_SETTINGS_MODULE](https://docs.djangoproject.com/en/topics/settings/#envvar-DJANGO_SETTINGS_MODULE) environment
variable.
...
```
    ...
    --traceback[¶](#cmdoption-traceback)

Displays a full stack trace when a [CommandError](https://docs.djangoproject.com/en/howto/custom-management-commands/#django.core.management.CommandError)
...
```
    ...
    --verbosity {0,1,2,3}, -v {0,1,2,3}[¶](#cmdoption-verbosity)
...
```
    ...
    --force-color[¶](#cmdoption-force-color)
...
colored output to another command.
   ...
   --skip-checks[¶](#cmdoption-skip-checks)

Skips running system checks prior to running the command. This option is only
available if the
[requires_system_checks](https://docs.djangoproject.com/en/howto/custom-management-commands/#django.core.management.BaseCommand.requires_system_checks) command
...
### Syntax coloring¶
   ...
   DJANGO_COLORS[¶](#envvar-DJANGO_COLORS)
...
See [How to create custom django-admin commands](https://docs.djangoproject.com/en/howto/custom-management-commands/) for how to add customized actions.
...
# Running management commands from your code¶
   ...
   django.core.management.call_command(*name*, **args*, ***options*)[¶](#django.core.management.call_command)
