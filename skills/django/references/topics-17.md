...
## ModelForm¶
   ...
   *class*ModelForm[[source]](https://docs.djangoproject.com/en/_modules/django/forms/models/#ModelForm)[¶](#django.forms.ModelForm)
...
| BigIntegerField | IntegerFieldwithmin_valueset to -9223372036854775808
andmax_valueset to 9223372036854775807. |
| BinaryField | CharField, ifeditableis set toTrueon the model field, otherwise not
represented in the form. |
| BooleanField | BooleanField, orNullBooleanFieldifnull=True. |
| CharField | CharFieldwithmax_lengthset to the model field’smax_lengthandempty_valueset toNoneifnull=True. |
| DateField | DateField |
...
field. See [Overriding the default fields](#overriding-the-default-fields) below.
...
### Validation on aModelForm¶

There are two main steps involved in validating a `ModelForm`:

1. [Validating the form](https://docs.djangoproject.com/en/ref/forms/validation/)
2. [Validating the model instance](https://docs.djangoproject.com/en/ref/models/instances/#validating-objects)

Just like normal form validation, model form validation is triggered implicitly
when calling [is_valid()](https://docs.djangoproject.com/en/ref/forms/api/#django.forms.Form.is_valid) or accessing the
[errors](https://docs.djangoproject.com/en/ref/forms/api/#django.forms.Form.errors) attribute and explicitly when calling
`full_clean()`, although you will typically not use the latter method in
practice.

`Model` validation ([Model.full_clean()](https://docs.djangoproject.com/en/ref/models/instances/#django.db.models.Model.full_clean)) is triggered from within the form
...
validation](https://docs.djangoproject.com/en/ref/models/instances/#validating-objects) step validate the uniqueness of model
...
fields. See the [form validation](https://docs.djangoproject.com/en/ref/forms/validation/) documentation
...
made. See [Validating objects](https://docs.djangoproject.com/en/ref/models/instances/#validating-objects) for more information
on the model’s `clean()` hook.

#### Considerations regarding model’serror_messages¶

Error messages defined at the
[formfield](https://docs.djangoproject.com/en/ref/forms/fields/#django.forms.Field.error_messages) level or at the
[form Meta](#modelforms-overriding-default-fields) level always take
precedence over the error messages defined at the
[modelfield](https://docs.djangoproject.com/en/ref/models/fields/#django.db.models.Field.error_messages) level.

Error messages  defined on [modelfields](https://docs.djangoproject.com/en/ref/models/fields/#django.db.models.Field.error_messages) are only used when the
`ValidationError` is raised during the [model validation](https://docs.djangoproject.com/en/ref/models/instances/#validating-objects) step and no corresponding error messages are defined at
the form level.

You can override the error messages from `NON_FIELD_ERRORS` raised by model
validation by adding the [NON_FIELD_ERRORS](https://docs.djangoproject.com/en/ref/exceptions/#django.core.exceptions.NON_FIELD_ERRORS) key
...
instance uses the model field [default](https://docs.djangoproject.com/en/ref/models/fields/#django.db.models.Field.default), if
there is one, for that field. This behavior doesn’t apply to fields that use
[CheckboxInput](https://docs.djangoproject.com/en/ref/forms/widgets/#django.forms.CheckboxInput),
[CheckboxSelectMultiple](https://docs.djangoproject.com/en/ref/forms/widgets/#django.forms.CheckboxSelectMultiple), or
[SelectMultiple](https://docs.djangoproject.com/en/ref/forms/widgets/#django.forms.SelectMultiple) (or any custom widget whose
[value_omitted_from_data()](https://docs.djangoproject.com/en/ref/forms/widgets/#django.forms.Widget.value_omitted_from_data) method always returns
...
[model saving options](https://docs.djangoproject.com/en/ref/models/instances/#ref-models-force-insert). `commit` is `True`
by default.
...
[Binding uploaded files to a form](https://docs.djangoproject.com/en/ref/forms/api/#binding-uploaded-files) for more information.
...
See the [section on saving forms](#the-save-method) for more details on using
`save(commit=False)`.

### Overriding the default fields¶

The default field types, as described in the [Field types](#field-types) table above, are
...
See the [form field documentation](https://docs.djangoproject.com/en/ref/forms/fields/) for more information
...
[modelform_factory()](https://docs.djangoproject.com/en/ref/forms/models/#django.forms.models.modelform_factory), instead of using a class
...
## Model formsets¶
   ...
   *class*models.BaseModelFormSet[¶](#django.forms.models.BaseModelFormSet)

Like [regular formsets](https://docs.djangoproject.com/en/5.0/topics/formsets/), Django provides a couple
...
[modelformset_factory()](https://docs.djangoproject.com/en/ref/forms/models/#django.forms.models.modelformset_factory) uses
[formset_factory()](https://docs.djangoproject.com/en/ref/forms/formsets/#django.forms.formsets.formset_factory) to generate formsets. This
means that a model formset is an extension of a basic formset that knows
how to interact with a particular model.

Note

When using [multi-table inheritance](https://docs.djangoproject.com/en/5.0/db/models/#multi-table-inheritance), forms
...
be created using [modelform_factory()](https://docs.djangoproject.com/en/ref/forms/models/#django.forms.models.modelform_factory).
...
### Specifying widgets to use in the form withwidgets¶
...
### Enabling localization for fields withlocalized_fields¶
...
As with regular formsets, it’s possible to [specify initial data](https://docs.djangoproject.com/en/5.0/topics/formsets/#formsets-initial-data) for forms in the formset by specifying an `initial`
parameter when instantiating the model formset class returned by
[modelformset_factory()](https://docs.djangoproject.com/en/ref/forms/models/#django.forms.models.modelformset_factory). However, with model
...
`ModelForms`, in [Selecting the fields to use](#selecting-the-fields-to-use).
...
containing the formset’s changes:
   ...
   models.BaseModelFormSet.changed_objects[¶](#django.forms.models.BaseModelFormSet.changed_objects)    models.BaseModelFormSet.deleted_objects[¶](#django.forms.models.BaseModelFormSet.deleted_objects)    models.BaseModelFormSet.new_objects[¶](#django.forms.models.BaseModelFormSet.new_objects)

### Limiting the number of editable objects¶

As with regular formsets, you can use the `max_num` and `extra` parameters
to [modelformset_factory()](https://docs.djangoproject.com/en/ref/forms/models/#django.forms.models.modelformset_factory) to limit the number of
...
[add additional forms with JavaScript](https://docs.djangoproject.com/en/5.0/topics/formsets/#understanding-the-managementform)
...
### Overridingclean()on aModelFormSet¶
...
form as shown above. See the [management form documentation](https://docs.djangoproject.com/en/5.0/topics/formsets/#understanding-the-managementform).
...
## Inline formsets¶
   ...
   *class*models.BaseInlineFormSet[¶](#django.forms.models.BaseInlineFormSet)
...
`BookFormSet`’s [prefix](https://docs.djangoproject.com/en/5.0/topics/formsets/#formset-prefix) is `'book_set'`
(`<model name>_set` ). If `Book`’s `ForeignKey` to `Author` has a
[related_name](https://docs.djangoproject.com/en/ref/models/fields/#django.db.models.ForeignKey.related_name), that’s used instead.

Note

[inlineformset_factory()](https://docs.djangoproject.com/en/ref/forms/models/#django.forms.models.inlineformset_factory) uses
[modelformset_factory()](https://docs.djangoproject.com/en/ref/forms/models/#django.forms.models.modelformset_factory) and marks
`can_delete=True`.

See also

[Manually rendered can_delete and can_order](https://docs.djangoproject.com/en/5.0/topics/formsets/#manually-rendered-can-delete-and-can-order).

### Overriding methods on anInlineFormSet¶
...
[inlineformset_factory()](https://docs.djangoproject.com/en/ref/forms/models/#django.forms.models.inlineformset_factory):
...
widgets](#specifying-widgets-to-use-in-the-form-with-widgets) above.
