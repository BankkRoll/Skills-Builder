...
With the Options API, we can use the [watchoption](https://vuejs.org/api/options-state#watch) to trigger a function whenever a reactive property changes:
...
With Composition API, we can use the [watchfunction](https://vuejs.org/api/reactivity-core#watch) to trigger a callback whenever a piece of reactive state changes:
