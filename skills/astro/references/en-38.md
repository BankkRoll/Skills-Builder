...
Need to upgrade an older project to v3? See our [older migration guide](https://docs.astro.build/en/guides/upgrade-to/v3/).
...
### Changed:app.rendersignature in Integrations API
...
### Removed: Shiki languagepathproperty
...
### Deprecated:handleFormsfor View Transitionssubmitevents
...
```
---import { ViewTransitions } from "astro:transitions";---<html>  <head>    <ViewTransitions handleForms />  </head>  <body>      </body></html>
```
...
### Removed:build.splitandbuild.excludeMiddleware

[Section titled “Removed: build.split and build.excludeMiddleware”](#removed-buildsplit-and-buildexcludemiddleware)

In Astro v3.0, `build.split` and `build.excludeMiddleware` build config options were deprecated and replaced with [adapter configuration options](https://docs.astro.build/en/reference/adapter-reference/#adapter-features) to perform the same tasks.
...
Please see the v3 migration guide to [update these deprecated middleware properties](https://docs.astro.build/en/guides/upgrade-to/v3/#deprecated-buildexcludemiddleware-and-buildsplit) with adapter configurations.
...
### Removed: usingrssingetStaticPaths()
...
If you are using the unsupported method for generating RSS feeds, you must now use the [@astrojs/rssintegration](https://docs.astro.build/en/recipes/rss/) for a complete RSS setup.
...
Please see the v3 migration guide [for guidance using uppercase HTTP request methods](https://docs.astro.build/en/guides/upgrade-to/v3/#changed-http-request-methods-case).

### Removed: 301 redirects when missing abaseprefix
...
When using the Astro preview server, all of your static asset imports and URLs from the public directory must have [the base value](https://docs.astro.build/en/reference/configuration-reference/#base) prefixed to the path.
...
### Removed:astro/client-imageauto-conversion
