...
Astro provides hooks that integrations can implement to execute during certain parts of Astro’s lifecycle. Astro hooks are defined in the `IntegrationHooks` interface, which is part of the global `Astro` namespace. Each hook has a [loggeroption](#astrointegrationlogger) that allows you to use the Astro logger to write logs.
...
A read-only copy of the user-supplied [Astro config](https://docs.astro.build/en/reference/configuration-reference/). This is resolved *before* any other integrations have run. If you need a copy of the config after all integrations have completed their config updates, [see theastro:config:donehook](#astroconfigdone).
...
Adds a [custom client directive](https://docs.astro.build/en/reference/directives-reference/#custom-client-directives) to be used in `.astro` files.
...
Adds a [custom dev toolbar app](https://docs.astro.build/en/reference/dev-toolbar-app-reference/).
...
Adds [middleware](https://docs.astro.build/en/guides/middleware/) to run on each request. Takes the `entrypoint` module that contains the middleware, and an `order` to specify whether it should run before (`pre`) other middleware or after (`post`).
...
Middleware is defined in a package with an [onRequest()function](https://docs.astro.build/en/reference/modules/astro-middleware/#onrequest), as with user-defined middleware.
...
A callback function to inject routes into an Astro project. Injected routes can be [.astropages](https://docs.astro.build/en/basics/astro-pages/) or [.jsand.tsroute handlers](https://docs.astro.build/en/guides/endpoints/#static-file-endpoints).
...
**Why:** To set options for a route at build or request time, such as enabling [on-demand server rendering](https://docs.astro.build/en/guides/on-demand-rendering/#enabling-on-demand-rendering).
...
The `prerender` property is used to configure [on-demand server rendering](https://docs.astro.build/en/guides/on-demand-rendering/#enabling-on-demand-rendering) for a route. If the route file contains an explicit `export const prerender` value, the value will be used as the default instead of `undefined`.
...
If the final value after running all the hooks is `undefined`, the route will fall back to a prerender default based on the [outputoption](https://docs.astro.build/en/reference/configuration-reference/#output): prerendered for `static` mode, and on-demand rendered for `server` mode.
...
A read-only copy of the user-supplied [Astro config](https://docs.astro.build/en/reference/configuration-reference/). This is resolved *after* other integrations have run.
...
Makes the integration an adapter. Read more in the [adapter API](https://docs.astro.build/en/reference/adapter-reference/).
...
A mutable instance of the Vite server used in “dev” mode. For instance, this is [used by our Partytown integration](https://docs.astro.build/en/guides/integrations-guide/partytown/) to inject the Partytown server as middleware:
...
An object providing callback functions to interact with the [dev toolbar](https://docs.astro.build/en/reference/dev-toolbar-app-reference/):
...
The loader can then access the `refreshContextData` property to get the webhook body. See the [refreshContextData](https://docs.astro.build/en/reference/content-loader-reference/#refreshcontextdata) property for more information.
...
**Why:** To set up any global objects or clients needed during a production build. This can also extend the build configuration options in the [adapter API](https://docs.astro.build/en/reference/adapter-reference/).
...
Exposes the [middleware](https://docs.astro.build/en/guides/middleware/) file path.
...
**Why:** To access generated routes and assets for extension (ex. copy content into the generated `/assets` directory). If you plan to transform generated assets, we recommend exploring the [Vite Plugin API](https://vite.dev/guide/api-plugin.html) and [configuring viaastro:config:setup](#updateconfig-option) instead.
...
```
import type {  AstroIntegrationLogger,  AstroIntegrationMiddleware,  AstroMiddlewareInstance,  AstroRenderer,  ClientDirectiveConfig,  HookParameters,  IntegrationResolvedRoute,  RedirectConfig,  RouteData,  RoutePart,  RouteType,  SSRComponentMetadata,  SSRLoadedRenderer,  SSRLoadedRendererValue,  SSRManifest,  ValidRedirectStatus,  // The following are deprecated:  IntegrationRouteData,} from "astro";
```
...
An instance of the Astro logger, useful to write logs. This logger uses the same [log level](https://docs.astro.build/en/reference/cli-reference/#--verbose)
configured via CLI.
...
An object containing an [onRequest()](https://docs.astro.build/en/reference/modules/astro-middleware/#onrequest) property defined with the project’s middleware function when it exists.
...
In the following example, a function’s `options` argument is typed to match the parameters of the [astro:config:setuphook](#astroconfigsetup):
...
```
interface IntegrationResolvedRoute extends Pick<    RouteData,    'generate' | 'params' | 'pathname' | 'segments' | 'type' | 'redirect' | 'origin'  > & {  pattern: RouteData['route'];  patternRegex: RouteData['pattern'];  entrypoint: RouteData['component'];  isPrerendered: RouteData['prerender'];  redirectRoute?: IntegrationResolvedRoute;}
```
...
Determines whether the route use [on demand rendering](https://docs.astro.build/en/guides/on-demand-rendering/). The value will be `true` for projects configured with:
...
Allows you to access the route `params`. For example, when a project uses the following [dynamic routes](https://docs.astro.build/en/guides/routing/#dynamic-routes) `/pages/[lang]/[...slug].astro`, the value will be `['lang', '...slug']`.
...
For regular routes, the value will be the URL pathname where this route will be served. When the project uses [dynamic routes](https://docs.astro.build/en/guides/routing/#dynamic-routes) (ie. `[dynamic]` or `[...spread]`), the pathname will be undefined.
...
Determines whether a route uses [on demand rendering](https://docs.astro.build/en/guides/on-demand-rendering/) or is statically prerendered at build time.

See also [prerendered](https://docs.astro.build/en/reference/routing-reference/#prerender) in the routing reference.
...
Defines a list of `RouteData` to fallback to when [i18n.fallback](https://docs.astro.build/en/reference/configuration-reference/#i18nfallback) has a list of locales.
...
Defines the name of the [server adapter](https://docs.astro.build/en/guides/on-demand-rendering/#server-adapters) used for on-demand rendering.
...
Defines a list of [HTMLlinkelement](https://developer.mozilla.org/en-US/docs/Web/HTML/Reference/Elements/link) required by this route.
...
Specifies the [configuredsite](https://docs.astro.build/en/reference/configuration-reference/#site).
...
Specifies the [configuredbasepath](https://docs.astro.build/en/reference/configuration-reference/#base) to deploy to.
...
**Type:** [AstroConfig['trailingSlash']](https://docs.astro.build/en/reference/configuration-reference/#trailingslash)

 **Added in:** `astro@3.5.4`

Specifies the [configured behavior for trailing slashes](https://docs.astro.build/en/reference/configuration-reference/#trailingslash) in development mode and for on-demand rendered pages.

#### SSRManifest.buildFormat

[Section titled “SSRManifest.buildFormat”](#ssrmanifestbuildformat)

**Type:** [NonNullable<AstroConfig['build']>['format']](https://docs.astro.build/en/reference/configuration-reference/#buildformat)

 **Added in:** `astro@4.2.2`

Specifies the [configured output file format](https://docs.astro.build/en/reference/configuration-reference/#buildformat).
...
Determines whether [HTML minification is enabled in the project configuration](https://docs.astro.build/en/reference/configuration-reference/#compresshtml).
...
Specifies the [configured prefix for Astro-generated asset links](https://docs.astro.build/en/reference/configuration-reference/#buildassetsprefix).
...
Defines a mapping of client directive names (e.g. `load`, `visible`) to their implementation code. This includes both [built-in client directives](https://docs.astro.build/en/reference/directives-reference/#client-directives) and [custom client directives](https://docs.astro.build/en/reference/directives-reference/#custom-client-directives).
...
**Type:** [MiddlewareHandler](https://docs.astro.build/en/reference/modules/astro-middleware/#middlewarehandler)

 **Added in:** `astro@3.0.3`

An [Astro middleware function](https://docs.astro.build/en/reference/modules/astro-middleware/#onrequest) when defined in the user project.
...
Specifies the resolved [i18nconfiguration](https://docs.astro.build/en/reference/configuration-reference/#i18n) when enabled in the project.
...
Defines the [i18n routing strategy](https://docs.astro.build/en/reference/configuration-reference/#i18nrouting) configured. This determines how locales are handled in URLs and whether redirects occur.
...
Specifies a list of [supported locales configured in the project](https://docs.astro.build/en/reference/configuration-reference/#i18nlocales).
...
Determines the [default locale configured in the project](https://docs.astro.build/en/reference/configuration-reference/#i18ndefaultlocale).
...
Specifies a mapping of locales to their fallback locales as [configured ini18n.fallback](https://docs.astro.build/en/reference/configuration-reference/#i18nfallback).
...
Determines the [configured fallback strategy for the project](https://docs.astro.build/en/reference/configuration-reference/#i18nroutingfallbacktype).
...
A mapping of [configured domains](https://docs.astro.build/en/reference/configuration-reference/#i18ndomains) to their associated locales.
...
Determines whether [origin checking is enabled in the security configuration](https://docs.astro.build/en/reference/configuration-reference/#securitycheckorigin).
...
Specifies the [configured list of permitted host patterns](https://docs.astro.build/en/reference/configuration-reference/#securityalloweddomains) for incoming requests when using on-demand rendering.
...
Specifies the [configured directory for caching build artifacts](https://docs.astro.build/en/reference/configuration-reference/#cachedir).
...
Specifies the [configured directory that Astro will read the site from](https://docs.astro.build/en/reference/configuration-reference/#srcdir).
...
Specifies the [configured directory in which to write the final build](https://docs.astro.build/en/reference/configuration-reference/#outdir).
...
Specifies the [configured directory for the static assets](https://docs.astro.build/en/reference/configuration-reference/#publicdir).
...
Describes the [Content Security Policy configuration](https://docs.astro.build/en/reference/experimental-flags/csp/).
...
Specifies whether CSP directives should be injected as a `meta` element, as a response `header`, or by the [adapterwhen it supports setting response headers](https://docs.astro.build/en/reference/adapter-reference/#experimentalstaticheaders).
...
Specifies the [configured hash function](https://docs.astro.build/en/reference/experimental-flags/csp/#algorithm).
...
Specifies a list of generated hashes for project scripts and [user-supplied hashes](https://docs.astro.build/en/reference/experimental-flags/csp/#hashes) for external scripts.
...
Specifies a list of valid sources combining the [configured script resources](https://docs.astro.build/en/reference/experimental-flags/csp/#resources) and the [injected script resources](https://docs.astro.build/en/reference/experimental-flags/csp/#cspinsertscriptresource).
...
Determines whether support for [dynamic script injection is enabled in the configuration](https://docs.astro.build/en/reference/experimental-flags/csp/#strictdynamic).
...
Specifies a list of generated hashes for project styles and [user-supplied hashes](https://docs.astro.build/en/reference/experimental-flags/csp/#hashes) for external styles.
...
Specifies a list of valid sources combining the [configured style resources](https://docs.astro.build/en/reference/experimental-flags/csp/#resources) and the [injected style resources](https://docs.astro.build/en/reference/experimental-flags/csp/#cspinsertstyleresource).
...
Specifies the [configured list of valid sources](https://docs.astro.build/en/reference/experimental-flags/csp/#directives) for specific content types.
...
```
type IntegrationRouteData = Omit<  RouteData,  'isIndex' | 'fallbackRoutes' | 'redirectRoute' | 'origin'> & {  redirectRoute?: IntegrationRouteData;};
```
...
## Allow installation withastro add

[Section titled “Allow installation with astro add”](#allow-installation-with-astro-add)

[Theastro addcommand](https://docs.astro.build/en/reference/cli-reference/#astro-add) allows users to easily add integrations and adapters to their project. If you want *your* integration to be installable with this tool, **addastro-integrationto thekeywordsfield in yourpackage.json**:
