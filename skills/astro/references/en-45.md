...
The `Astro` global object is available to all `.astro` files. Use the `context` object in [endpoint functions](https://docs.astro.build/en/guides/endpoints/) to serve static or live server endpoints and in [middleware](https://docs.astro.build/en/guides/middleware/) to inject behavior when a page or endpoint is about to be rendered.
...
- [Astro.props](#tab-panel-1947)
- [context.props](#tab-panel-1948)
...
See also: [Data Passing withprops](https://docs.astro.build/en/reference/routing-reference/#data-passing-with-props)
...
- [Astro.params](#tab-panel-1949)
- [context.params](#tab-panel-1950)
...
See also: [params](https://docs.astro.build/en/reference/routing-reference/#params)
...
- [Astro.clientAddress](#tab-panel-1951)
- [context.clientAddress](#tab-panel-1952)
...
- [Astro.generator](#tab-panel-1953)
- [context.generator](#tab-panel-1954)
...
- [Astro.request](#tab-panel-1955)
- [context.request](#tab-panel-1956)
...
- [Astro.redirect()](#tab-panel-1957)
- [context.redirect()](#tab-panel-1958)
...
- [Astro.rewrite()](#tab-panel-1959)
- [context.rewrite()](#tab-panel-1960)
...
- [Astro.rewrite()](#tab-panel-1961)
- [context.rewrite()](#tab-panel-1962)
...
- [Astro.rewrite()](#tab-panel-1963)
- [context.rewrite()](#tab-panel-1964)
...
- [Astro.originPathname](#tab-panel-1965)
- [context.originPathname](#tab-panel-1966)
...
- [Astro.locals](#tab-panel-1967)
- [context.locals](#tab-panel-1968)
...
It is computed by checking the configured locales in your [i18n.locales](https://docs.astro.build/en/reference/configuration-reference/#i18nlocales) array and the locales supported by the user’s browser via the header `Accept-Language`. This value is `undefined` if no such match exists.
...
If the browser does not specify any preferred languages, then this value will be [i18n.locales](https://docs.astro.build/en/reference/configuration-reference/#i18nlocales): all of your supported locales will be considered equally preferred by a visitor with no preferences.
...
The locale computed from the current URL, using the syntax specified in your `locales` configuration. If the URL does not contain a `/[locale]/` prefix, then the value will default to [i18n.defaultLocale](https://docs.astro.build/en/reference/configuration-reference/#i18ndefaultlocale).
...
`getActionResult()` is a function that returns the result of an [Action](https://docs.astro.build/en/guides/actions/) submission. This accepts an action function as an argument (e.g. `actions.logout`) and returns a `data` or `error` object when a submission is received. Otherwise, it will return `undefined`.
...
`session` is an object that allows data to be stored between requests for [routes rendered on demand](https://docs.astro.build/en/guides/on-demand-rendering/). It is associated with a cookie that contains the session ID only: the data itself is not stored in the cookie.
...
See [the session guide](https://docs.astro.build/en/guides/sessions/) for more information on how to use sessions in your Astro project.
...
- [Astro.session](#tab-panel-1969)
- [context.session](#tab-panel-1970)
...
- [Astro.session](#tab-panel-1971)
- [context.session](#tab-panel-1972)
...
- [Astro.session](#tab-panel-1973)
- [context.session](#tab-panel-1974)
...
- [Astro.session](#tab-panel-1975)
- [context.session](#tab-panel-1976)
...
- [Astro.session](#tab-panel-1977)
- [context.session](#tab-panel-1978)
...
`.glob()` can’t take variables or strings that interpolate them, as they aren’t statically analyzable. (See [the imports guide](https://docs.astro.build/en/guides/imports/#supported-values) for a workaround.) This is because `Astro.glob()` is a wrapper of Vite’s [import.meta.glob()](https://vite.dev/guide/features.html#glob-import).
...
```
---const name = "Astro";---<div>  <h1>Hello {name}!</h1>  </div>
```
...
```
---import MyComponent from "./MyComponent.astro";const Element = 'div'const Component = MyComponent;---<Element>Hello!</Element> <Component /> 
```
...
- **Thedefine:vars directiveis not supported.** If you cannot wrap the children with an extra element (e.g `<div>`), then you can manually add a `style={`--myVar:${value}`}` to your Element.
...
```
---const { items } = Astro.props;---<ul class="nested-list">  {items.map((item) => (    <li>                  {Array.isArray(item) ? (        <Astro.self items={item} />      ) : (        item      )}    </li>  ))}</ul>
```
