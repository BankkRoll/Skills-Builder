...
The following scripts for the most common `astro` commands (`astro dev`, `astro build`, and `astro preview`) are added for you automatically when you create a project using [thecreate astrowizard](https://docs.astro.build/en/install-and-setup/).

When you follow the instructions to [install Astro manually](https://docs.astro.build/en/install-and-setup/#manual-setup), you are instructed to add these scripts yourself. You can also add more scripts to this list manually for any commands you use frequently.
...
Builds your site for deployment. By default, this will generate static files and place them in a `dist/` directory. If any routes are [rendered on demand](https://docs.astro.build/en/guides/on-demand-rendering/), this will generate the necessary server files to serve your site.
...
This command allows you to preview your site locally [after building](#astro-build) to catch any errors in your build output before deploying it. It is not designed to be run in production. For help with production hosting, check out our guide on [Deploying an Astro Website](https://docs.astro.build/en/guides/deploy/).
...
Specifies not to run `astro sync` before checking the project.
   ...
   Read more about [type checking in Astro](https://docs.astro.build/en/guides/typescript/#type-checking).
...
Generates TypeScript types for all Astro modules. This sets up a [.astro/types.d.tsfile](https://docs.astro.build/en/guides/typescript/#setup) for type inferencing, and defines modules for features that rely on generated types:

- The `astro:content` module for the [Content Collections API](https://docs.astro.build/en/guides/content-collections/).
- The `astro:db` module for [Astro DB](https://docs.astro.build/en/guides/astro-db/).
- The `astro:env` module for [Astro Env](https://docs.astro.build/en/guides/environment-variables/).
- The `astro:actions` module for [Astro Actions](https://docs.astro.build/en/guides/actions/)

## astro add

[Section titled “astro add”](#astro-add)

Adds an integration to your configuration. Read more in [the integrations guide](https://docs.astro.build/en/guides/integrations-guide/#automatic-integration-setup).
...
Set this key as the `ASTRO_KEY` environment variable (e.g. in a `.env` file) and include it in your CI/CD or host’s build settings when you need [a constant encryption key for your server islands](https://docs.astro.build/en/guides/server-islands/#reusing-the-encryption-key) for situations like rolling deployments, multi-region hosting or a CDN that caches pages containing server islands.
...
Clear the [content layer cache](https://docs.astro.build/en/guides/content-collections/#defining-the-collection-loader), forcing a full rebuild.
...
Configures the [mode](https://docs.astro.build/en/reference/programmatic-reference/#mode) inline config for your project.
...
Configures the [outDir](https://docs.astro.build/en/reference/configuration-reference/#outdir) for your project. Passing this flag will override the `outDir` value in your `astro.config.mjs` file, if one exists.

### --site <url>

[Section titled “--site <url>”](#--site-url)

Configures the [site](https://docs.astro.build/en/reference/configuration-reference/#site) for your project. Passing this flag will override the `site` value in your `astro.config.mjs` file, if one exists.
...
Configures the [base](https://docs.astro.build/en/reference/configuration-reference/#base) for your project. Passing this flag will override the `base` value in your `astro.config.mjs` file, if one exists.
...
Refer to [Vite’sallowedHostsfeature](https://vite.dev/config/server-options.html#server-allowedhosts) for more information, including security implications of allowing hostnames.
