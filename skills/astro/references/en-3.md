...
- Automatically validate JSON and form data inputs using [Zod validation](https://docs.astro.build/en/reference/modules/astro-zod/).
...
2. Import the `defineAction()` utility from `astro:actions`, and the `z` object from `astro/zod`.
  ...
  export const server = {  // action declarations}
  ```
3. Use the `defineAction()` utility to define a `getGreeting` action. The `input` property will be used to validate input parameters with a [Zod schema](https://docs.astro.build/en/reference/modules/astro-zod/#common-data-type-validators) and the `handler()` function includes the backend logic to run on the server.
   src/actions/index.ts
  ```
  import { defineAction } from 'astro:actions';import { z } from 'astro/zod';
...
Actions return a custom data format that can handle Dates, Maps, Sets, and URLs [using the Devalue library](https://github.com/Rich-Harris/devalue). Therefore, you can’t easily inspect the response from the network like you can with regular JSON. For debugging, you can instead inspect the `data` object returned by actions.
   ...
   [See thehandler()API reference](https://docs.astro.build/en/reference/modules/astro-actions/#handler-property) for full details.
...
### Accessingdatadirectly without an error check
...
#### Creating anActionError

[Section titled “Creating an ActionError”](#creating-an-actionerror)

To throw an error, import the [ActionError()class](https://docs.astro.build/en/reference/modules/astro-actions/#actionerror) from the `astro:actions` module. Pass it a human-readable status `code` (e.g. `"NOT_FOUND"` or `"BAD_REQUEST"`), and an optional `message` to provide further information about the error.
...
2. Define a `newsletter` action to handle the submitted form. Validate the `email` field using the `z.string().email()` validator, and the `terms` checkbox using `z.boolean()`:
  ...
  ```
     See the [inputAPI reference](https://docs.astro.build/en/reference/modules/astro-actions/#input-validator) for all available form validators.
...
For more customization, you can [use theisInputError()utility](#displaying-form-input-errors) to check whether an error is caused by invalid input.
...
Actions expose [a subset of theAPIContextobject](https://docs.astro.build/en/reference/modules/astro-actions/#actionapicontext) to access properties passed from middleware using `context.locals`. When a user is not authorized, you can raise an `ActionError` with the `UNAUTHORIZED` code:
