# Astro Container API (experimental) and more
...
This experimental server-side API unlocks a variety of potential future uses, but is currently scoped to allow [testing of.astrocomponent output](https://docs.astro.build/en/guides/testing/#vitest-and-container-api) in `vite` environments such as `vitest`.
...
Enables rendering components using [HTML streaming](https://docs.astro.build/en/guides/on-demand-rendering/#html-streaming).
...
A list of loaded client renderers required by the component. Use this if your `.astro` component renders any [UI framework components](https://docs.astro.build/en/guides/framework-components/) or MDX using an official Astro integration (e.g. React, Vue, etc.).
...
For [on-demand rendered applications](https://docs.astro.build/en/guides/on-demand-rendering/), or cases where the container is called at runtime or inside other “shells” (e.g. PHP, Ruby, Java, etc.), renderers must be manually imported.
...
Server renderers are required to build your project, and must be stored in the container for every framework used. Client renderers are additionally needed to any hydrate client-side components using [client:*directives](https://docs.astro.build/en/reference/directives-reference/#client-directives).
...
An option to pass content to be rendered with [<slots>](https://docs.astro.build/en/basics/astro-components/#slots).
...
An option to pass [properties](https://docs.astro.build/en/basics/astro-components/#component-props) for Astro components.
...
An object to pass information about the path parameter to an Astro component responsible for [generating dynamic routes](https://docs.astro.build/en/guides/routing/#dynamic-routes).
...
An option to pass information from [Astro.locals](https://docs.astro.build/en/reference/api-reference/#locals) for rendering your component.
...
An option available when using `renderToResponse()` to specify that you are rendering an [endpoint](https://docs.astro.build/en/guides/endpoints/):
...
Whether or not the Container API renders components as if they were [page partials](https://docs.astro.build/en/basics/astro-pages/#page-partials). The default `true` setting renders the component in isolation without a full page shell.
...
When `retainBody` is `false`, [entry.body](https://docs.astro.build/en/reference/modules/astro-content/#body) will be `undefined` instead of containing the raw file contents.
...
For Markdown files, the rendered body will still be available in the [entry.rendered.htmlproperty](https://docs.astro.build/en/reference/content-loader-reference/#rendered), and the [entry.filePathproperty](https://docs.astro.build/en/reference/content-loader-reference/#filepath) will still point to the original file.
...
A callback function to create a collection from a file’s contents. Use it when you need to process file not supported by default (e.g. `.csv`) or when using [nested.jsondocuments](https://docs.astro.build/en/guides/content-collections/#nested-json-documents).
...
Assign this object to the [rendered](#rendered) field of the [DataEntry](#dataentry) object to allow users to [render the content in a page](https://docs.astro.build/en/guides/content-collections/#rendering-body-content).
...
Generates a non-cryptographic content digest of an object or string. This can be used to track if the data has changed by setting [thedigestfield](#digest) of an entry.
...
If not set, then any fields in the schema that use [theimage()helper](https://docs.astro.build/en/guides/images/#images-in-content-collections) will be treated as [public paths](https://docs.astro.build/en/guides/images/#where-to-store-images) and not transformed.
...
If this field is provided, then [therender()function and<Content />component](https://docs.astro.build/en/guides/content-collections/#rendering-body-content) are available to render the entry in a page.
