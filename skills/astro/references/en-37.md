...
Need to upgrade an older project to v2? See our [older migration guide](https://docs.astro.build/en/guides/upgrade-to/v2/).
...
Check that both your development environment and your deployment environment are using **Node18.14.1or higher**.
...
1. Check your local version of Node using:
  ...
  node -v
  ```
2. Check your [deployment environment’s](https://docs.astro.build/en/guides/deploy/) own documentation to verify that they support Node 18.
...
Otherwise, delete all references to importing Astro’s `<Markdown />` component and the component itself in your `.astro` files. You will need to rewrite your content as HTML directly or [import Markdown](https://docs.astro.build/en/guides/markdown-content/#importing-markdown) from a `.md` file.
...
If you are using Web APIs in server-rendered components, you will need to either make the usage of those APIs conditional or use [theclient:onlyclient directive](https://docs.astro.build/en/reference/directives-reference/#clientonly).

### Removed:imagefromastro:contentin content collections schema
...
If you are using the deprecated `image()` from `astro:content`, remove it as this no longer exists. Validate images through [theimagehelper fromschema](#update-content-collections-schemas) instead:
...
### Removed:class:listfeatures

[Section titled “Removed: class:list features”](#removed-classlist-features)

In Astro v2.x, the [class:listdirective](https://docs.astro.build/en/reference/directives-reference/#classlist) used a custom implementation inspired by [clsx](https://github.com/lukeed/clsx) with a few extra features like deduplication and `Set` support.
...
### Removed: passingclass:listas a prop

[Section titled “Removed: passing class:list as a prop”](#removed-passing-classlist-as-a-prop)

In Astro v2.x, [class:listvalues](https://docs.astro.build/en/reference/directives-reference/#classlist) were sent to components via [Astro.props['class:list']](https://docs.astro.build/en/reference/api-reference/#props).
...
In Astro v2.x, camelCase [CSS variables](https://docs.astro.build/en/guides/styling/#css-variables) passed to the `style` attribute were rendered as both camelCase (as written) and kebab-case (kept for backwards compatibility).
...
```
---const myValue = "red"---<div style={{ "--myValue": myValue }}></div>
<div style="--my-value:var(--myValue);--myValue:red"></div><div style="--myValue:red"></div>
```
...
### Removed: automatic flattening ofgetStaticPaths()’s return value
...
In Astro v2.x, the return value of [getStaticPaths()](https://docs.astro.build/en/reference/routing-reference/#getstaticpaths) was automatically flattened to allow you to return an array of arrays without errors.
...
An [error message indicating thatgetStaticPath()’s return value must be an array of objects](https://docs.astro.build/en/reference/errors/invalid-get-static-paths-entry/#what-went-wrong) will be provided if you need to update your code.

### Moved:astro checknow requires an external package

[Section titled “Moved: astro check now requires an external package”](#moved-astro-check-now-requires-an-external-package)

In Astro v2.x, [astro check](https://docs.astro.build/en/reference/cli-reference/#astro-check) was included in Astro by default, and its dependencies were bundled in Astro. This meant a larger package whether or not you ever used `astro check`. This also prevented you from having control over the version of TypeScript and the Astro Language Server to use.
...
### Deprecated:build.excludeMiddlewareandbuild.split
...
Astro v3.0 replaces these build config options with new [SSR adapter configuration options](https://docs.astro.build/en/guides/integrations-guide/#official-integrations) to perform the same tasks: `edgeMiddleware` and `functionPerRoute`.
...
To continue to mark some pages in your project as drafts, [migrate to content collections](https://docs.astro.build/en/guides/content-collections/) and manually filter out pages with the `draft: true` frontmatter property instead.
...
### Changed default:verbatimModuleSyntaxin tsconfig.json presets
...
### Changed default: port3000

[Section titled “Changed default: port 3000”](#changed-default-port-3000)

In Astro v2.x, Astro ran on port `3000` by default.

Astro v3.0 changes the [default port](https://docs.astro.build/en/reference/cli-reference/#--port-number) to `4321`. 🚀
...
### Changed default: import.meta.env.BASE_URLtrailingSlash
...
In Astro v2.x, `import.meta.env.BASE_URL` appended your [base](https://docs.astro.build/en/reference/configuration-reference/#base) setting with a [trailingSlash](https://docs.astro.build/en/reference/configuration-reference/#trailingslash) by default. `trailingSlash: "ignore"` also appended a trailing slash.
...
In Astro v2.x, Astro only compressed your emitted HTML when [compressHTML](https://docs.astro.build/en/reference/configuration-reference/#compresshtml) was explicitly set to `true`. The default value was `false`.
...
In Astro v2.x, the default value of [scopedStyleStrategy](https://docs.astro.build/en/reference/configuration-reference/#scopedstylestrategy) was `"where"`.
...
To retain your project’s current [style scoping](https://docs.astro.build/en/guides/styling/#scoped-styles), update the configuration file to the previous default value:
...
In Astro v2.x, Squoosh was the [default image processing service](https://docs.astro.build/en/guides/images/#default-image-service).
...
In Astro v2.x, [HTTP request methods](https://docs.astro.build/en/guides/endpoints/#http-methods) were written using lowercase function names: `get`, `post`, `put`, `all`, and `del`.
...
In Astro v2.x, you could use [multiple JSX framework integrations](https://docs.astro.build/en/guides/integrations-guide/#official-integrations) (React, Solid, Preact) in the same project without needing to identify which files belonged to which framework.
...
### Changed:Astro.cookies.get(key)can returnundefined
...
To [run the Astro CLI programmatically](https://docs.astro.build/en/reference/programmatic-reference/), use the new experimental JavaScript APIs:
...
#### Upgrade fromexperimental.assets
...
##### Remove the~/assetsimport alias

[Section titled “Remove the ~/assets import alias”](#remove-the-assets-import-alias)

This import alias is no longer included by default with `astro:assets`. If you were using this alias with experimental assets, you must convert them to relative file paths, or [create your own import aliases](https://docs.astro.build/en/guides/imports/#aliases).
...
You must update the `src` attribute of any existing `<img>` tags (including any [images in UI framework components](https://docs.astro.build/en/guides/images/#images-in-ui-framework-components)) and you may also update other attributes that are now available to you from the imported image.
...
If you require more control over your image attributes, we recommend using the `.mdx` file format, which allows you to include Astro’s `<Image />` component or a JSX `<img />` tag in addition to the Markdown syntax. Use the [MDX integration](https://docs.astro.build/en/guides/integrations-guide/mdx/) to add support for MDX to Astro.
...
1. Remove the `@astrojs/image` integration.
  You must [remove the integration](https://docs.astro.build/en/guides/integrations-guide/#removing-an-integration) by uninstalling and then removing it from your `astro.config.mjs` file.
   astro.config.mjs
  ```
  import { defineConfig } from 'astro/config';import image from '@astrojs/image';
  export default defineConfig({  integrations: [    image(),  ]})
...
3. Migrate any existing `<Image />` components.
  ...
  Remove any component attributes that are not [currently supported image asset properties](https://docs.astro.build/en/reference/modules/astro-assets/#image-).
...
The new `image` helper for content collections lets you validate the image metadata using Zod. Learn more about [how to use images in content collections](https://docs.astro.build/en/guides/images/#images-in-content-collections)
...
Please follow the instructions below as appropriate to upgrade **an Astro v2.x project configured withexperimental.viewTransitions: true**  to v3.0.

#### Upgrade fromexperimental.viewTransitions
...
##### Removeexperimental.viewTransitionsflag
...
#### Updatetransition:animatedirectives
