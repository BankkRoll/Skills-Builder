...
such as [Session.weak_identity_map](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session.params.weak_identity_map) and classes such as
`MapperExtension`.     While all deprecations have been noted in the
...
[“threadlocal” engine strategy deprecated](#change-4393-threadlocal)

[convert_unicode parameters deprecated](#change-4393-convertunicode)

[Relationship to AliasedClass replaces the need for non primary mappers](#change-4423)
...
The “non primary mapper” is a [Mapper](https://docs.sqlalchemy.org/en/20/orm/mapping_api.html#sqlalchemy.orm.Mapper) created in the
[Imperative Mapping](https://docs.sqlalchemy.org/en/20/orm/mapping_styles.html#orm-imperative-mapping) style, which acts as an additional mapper against an
...
anticipated that the [Mapper](https://docs.sqlalchemy.org/en/20/orm/mapping_api.html#sqlalchemy.orm.Mapper) object was to be the primary query
construction interface, before the [Query](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query) object existed.

With the advent of [Query](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query) and later the [AliasedClass](https://docs.sqlalchemy.org/en/20/orm/queryguide/api.html#sqlalchemy.orm.util.AliasedClass)
...
some very hard-to-define [relationship()](https://docs.sqlalchemy.org/en/20/orm/relationship_api.html#sqlalchemy.orm.relationship) configurations could be made
...
[relationship.primaryjoin](https://docs.sqlalchemy.org/en/20/orm/relationship_api.html#sqlalchemy.orm.relationship.params.primaryjoin) that encompassed all the complexity of a
...
in the old 0.1 - 0.4 days, one would use [Table](https://docs.sqlalchemy.org/en/20/core/metadata.html#sqlalchemy.schema.Table) objects directly with
the ORM).

The missing piece was to allow the [relationship()](https://docs.sqlalchemy.org/en/20/orm/relationship_api.html#sqlalchemy.orm.relationship) to refer directly
to the [AliasedClass](https://docs.sqlalchemy.org/en/20/orm/queryguide/api.html#sqlalchemy.orm.util.AliasedClass).  The [AliasedClass](https://docs.sqlalchemy.org/en/20/orm/queryguide/api.html#sqlalchemy.orm.util.AliasedClass) already does
...
were formerly for non primary mappers at [Configuring how Relationship Joins](https://docs.sqlalchemy.org/en/20/orm/join_conditions.html#relationship-configure-joins)
are changed to aliased class.

At [Relationship to Aliased Class](https://docs.sqlalchemy.org/en/20/orm/join_conditions.html#relationship-aliased-class), the original non primary mapper looked
like:
...
[relationship.omit_join](https://docs.sqlalchemy.org/en/20/orm/relationship_api.html#sqlalchemy.orm.relationship.params.omit_join) which can be set to `False` on the
[relationship()](https://docs.sqlalchemy.org/en/20/orm/relationship_api.html#sqlalchemy.orm.relationship) to disable the optimization.
...
[Session](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session), it would raise an error:
...
[Session](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session) is committed and the `u1` instance falls out of scope,
as the `Address.user == u1` expression does not strongly reference the
object itself, only its [InstanceState](https://docs.sqlalchemy.org/en/20/orm/internals.html#sqlalchemy.orm.InstanceState).
...
[InstanceState](https://docs.sqlalchemy.org/en/20/orm/internals.html#sqlalchemy.orm.InstanceState) which will memoize the last known value for a
particular attribute on that state when that attribute is expired.  This
mechanism is only enabled for a specific attribute / [InstanceState](https://docs.sqlalchemy.org/en/20/orm/internals.html#sqlalchemy.orm.InstanceState)
...
[InstanceState](https://docs.sqlalchemy.org/en/20/orm/internals.html#sqlalchemy.orm.InstanceState) that tracks the last known value of a column attribute
...
[DetachedInstanceError](https://docs.sqlalchemy.org/en/20/orm/exceptions.html#sqlalchemy.orm.exc.DetachedInstanceError) is no longer raised.
...
the [relationship.active_history](https://docs.sqlalchemy.org/en/20/orm/relationship_api.html#sqlalchemy.orm.relationship.params.active_history) flag, an assertion will not
...
a [DetachedInstanceError](https://docs.sqlalchemy.org/en/20/orm/exceptions.html#sqlalchemy.orm.exc.DetachedInstanceError) would be raised as the attribute is attempting
...
Added the `.info` dictionary to the [InstanceState](https://docs.sqlalchemy.org/en/20/orm/internals.html#sqlalchemy.orm.InstanceState) class, the object
that comes from calling [inspect()](https://docs.sqlalchemy.org/en/20/core/inspection.html#sqlalchemy.inspect) on a mapped object.  This allows custom
...
The [ShardedQuery](https://docs.sqlalchemy.org/en/20/orm/extensions/horizontal_shard.html#sqlalchemy.ext.horizontal_shard.ShardedQuery) extension object supports the [Query.update()](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query.update)
and [Query.delete()](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query.delete) bulk update/delete methods.    The `query_chooser`
...
[AssociationProxy.cascade_scalar_deletes](https://docs.sqlalchemy.org/en/20/orm/extensions/associationproxy.html#sqlalchemy.ext.associationproxy.AssociationProxy.params.cascade_scalar_deletes).  When set, setting `A.b`
...
The [AssociationProxy](https://docs.sqlalchemy.org/en/20/orm/extensions/associationproxy.html#sqlalchemy.ext.associationproxy.AssociationProxy) object makes lots of decisions based on the
parent mapped class it is associated with.   While the
[AssociationProxy](https://docs.sqlalchemy.org/en/20/orm/extensions/associationproxy.html#sqlalchemy.ext.associationproxy.AssociationProxy) historically began as a relatively simple ‘getter,’
...
[AssociationProxy](https://docs.sqlalchemy.org/en/20/orm/extensions/associationproxy.html#sqlalchemy.ext.associationproxy.AssociationProxy) object would therefore store state that is specific
to that class, but only once this method were called; trying to inspect this
state ahead of time without first accessing the [AssociationProxy](https://docs.sqlalchemy.org/en/20/orm/extensions/associationproxy.html#sqlalchemy.ext.associationproxy.AssociationProxy) as a
...
These problems are now solved in that [AssociationProxy](https://docs.sqlalchemy.org/en/20/orm/extensions/associationproxy.html#sqlalchemy.ext.associationproxy.AssociationProxy) no longer
modifies its own internal state when `__get__()` is called; instead, a new
object is generated per-class known as [AssociationProxyInstance](https://docs.sqlalchemy.org/en/20/orm/extensions/associationproxy.html#sqlalchemy.ext.associationproxy.AssociationProxyInstance) which
handles all the state specific to a particular mapped parent class (when the
parent class is not mapped, no [AssociationProxyInstance](https://docs.sqlalchemy.org/en/20/orm/extensions/associationproxy.html#sqlalchemy.ext.associationproxy.AssociationProxyInstance) is generated).
...
[AssociationProxy](https://docs.sqlalchemy.org/en/20/orm/extensions/associationproxy.html#sqlalchemy.ext.associationproxy.AssociationProxy) without necessarily calling `__get__()`, a new
method [AssociationProxy.for_class()](https://docs.sqlalchemy.org/en/20/orm/extensions/associationproxy.html#sqlalchemy.ext.associationproxy.AssociationProxy.for_class) is added that provides direct access
to a class-specific [AssociationProxyInstance](https://docs.sqlalchemy.org/en/20/orm/extensions/associationproxy.html#sqlalchemy.ext.associationproxy.AssociationProxyInstance), demonstrated as:
...
Once we have the [AssociationProxyInstance](https://docs.sqlalchemy.org/en/20/orm/extensions/associationproxy.html#sqlalchemy.ext.associationproxy.AssociationProxyInstance) object, in the above
...
Given an [AssociationProxy](https://docs.sqlalchemy.org/en/20/orm/extensions/associationproxy.html#sqlalchemy.ext.associationproxy.AssociationProxy) where the target is a database column,
...
Note that the [ColumnOperators.contains()](https://docs.sqlalchemy.org/en/20/core/sqlelement.html#sqlalchemy.sql.expression.ColumnOperators.contains) operator is in fact a string
...
part of [AssociationProxy stores class-specific state on a per-class basis](#change-3423); as the proxy now spins off additional state when
an expression is generated, there is both an object-target and a column-target
version of the [AssociationProxyInstance](https://docs.sqlalchemy.org/en/20/orm/extensions/associationproxy.html#sqlalchemy.ext.associationproxy.AssociationProxyInstance) class.
...
particular [Session](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session), it will remain part of that session’s  state
...
Python `list` as is the default for [relationship()](https://docs.sqlalchemy.org/en/20/orm/relationship_api.html#sqlalchemy.orm.relationship), contained
...
an ORM-mapped [relationship()](https://docs.sqlalchemy.org/en/20/orm/relationship_api.html#sqlalchemy.orm.relationship) that uses a sequence collection can have
...
During the above operation, interception of the standard Python `__setitem__` `__delitem__` methods delivers an interim state where the second `B()`
...
This change applies specifically to the use of the [joinedload()](https://docs.sqlalchemy.org/en/20/orm/queryguide/relationships.html#sqlalchemy.orm.joinedload) loading
strategy in conjunction with a row limited query, e.g. using [Query.first()](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query.first)
or [Query.limit()](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query.limit), as well as with use of the [Query.with_for_update()](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query.with_for_update) method.
...
The [Query](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query) object renders a SELECT of the following form when joined
...
However, MySQL due to [https://bugs.mysql.com/bug.php?id=90693](https://bugs.mysql.com/bug.php?id=90693) does not lock
...
When using the [Query.with_for_update.of](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query.with_for_update.params.of) modifier, typically on
...
The [relationship.passive_deletes](https://docs.sqlalchemy.org/en/20/orm/relationship_api.html#sqlalchemy.orm.relationship.params.passive_deletes) option accepts the value
...
To suit the case where a [MetaData](https://docs.sqlalchemy.org/en/20/core/metadata.html#sqlalchemy.schema.MetaData) naming convention needs to
...
separator.  Below we define a convention that will name [UniqueConstraint](https://docs.sqlalchemy.org/en/20/core/constraints.html#sqlalchemy.schema.UniqueConstraint)
...
Note that the truncation logic also raises [IdentifierError](https://docs.sqlalchemy.org/en/20/core/exceptions.html#sqlalchemy.exc.IdentifierError) when a
constraint name is explicitly too large for a given dialect.  This has been
the behavior for an [Index](https://docs.sqlalchemy.org/en/20/core/constraints.html#sqlalchemy.schema.Index) object for a long time, but is now applied
...
[conv()](https://docs.sqlalchemy.org/en/20/core/constraints.html#sqlalchemy.schema.conv) construct:
...
database-side truncation.  This has already been the case for [Index](https://docs.sqlalchemy.org/en/20/core/constraints.html#sqlalchemy.schema.Index)
...
object, suitable for usage in an ORM [relationship()](https://docs.sqlalchemy.org/en/20/orm/relationship_api.html#sqlalchemy.orm.relationship), by first
creating the function as usual using the [func](https://docs.sqlalchemy.org/en/20/core/sqlelement.html#sqlalchemy.sql.expression.func) factory, then
when the function is complete calling upon the [FunctionElement.as_comparison()](https://docs.sqlalchemy.org/en/20/core/functions.html#sqlalchemy.sql.functions.FunctionElement.as_comparison)
modifier to produce a [BinaryExpression](https://docs.sqlalchemy.org/en/20/core/sqlelement.html#sqlalchemy.sql.expression.BinaryExpression) that has a “left” and a “right”
side:
...
Above, the [relationship.primaryjoin](https://docs.sqlalchemy.org/en/20/orm/relationship_api.html#sqlalchemy.orm.relationship.params.primaryjoin) of the “descendants” relationship
...
The “expanding IN” feature introduced in version 1.2 at [Late-expanded IN parameter sets allow IN expressions with cached statements](https://docs.sqlalchemy.org/en/20/changelog/migration_12.html#change-3953) now
supports empty lists passed to the [ColumnOperators.in_()](https://docs.sqlalchemy.org/en/20/core/sqlelement.html#sqlalchemy.sql.expression.ColumnOperators.in_) operator.   The implementation
...
The [TypeEngine.bind_expression()](https://docs.sqlalchemy.org/en/20/core/type_api.html#sqlalchemy.types.TypeEngine.bind_expression) and [TypeEngine.column_expression()](https://docs.sqlalchemy.org/en/20/core/type_api.html#sqlalchemy.types.TypeEngine.column_expression) methods
now work when they are present on the “impl” of a particular datatype, allowing these methods
to be used by dialects as well as for [TypeDecorator](https://docs.sqlalchemy.org/en/20/core/custom_types.html#sqlalchemy.types.TypeDecorator) and [Variant](https://docs.sqlalchemy.org/en/20/core/type_api.html#sqlalchemy.types.Variant) use cases.

The following example illustrates a [TypeDecorator](https://docs.sqlalchemy.org/en/20/core/custom_types.html#sqlalchemy.types.TypeDecorator) that applies SQL-time conversion
functions to a [LargeBinary](https://docs.sqlalchemy.org/en/20/core/type_basics.html#sqlalchemy.types.LargeBinary).   In order for this type to work in the
context of a [Variant](https://docs.sqlalchemy.org/en/20/core/type_api.html#sqlalchemy.types.Variant), the compiler needs to drill into the “impl” of the
...
[TypeEngine.bind_expression()](https://docs.sqlalchemy.org/en/20/core/type_api.html#sqlalchemy.types.TypeEngine.bind_expression) and [TypeEngine.column_expression()](https://docs.sqlalchemy.org/en/20/core/type_api.html#sqlalchemy.types.TypeEngine.column_expression)
...
The connection pool usually used by [create_engine()](https://docs.sqlalchemy.org/en/20/core/engines.html#sqlalchemy.create_engine) is known
as [QueuePool](https://docs.sqlalchemy.org/en/20/core/pooling.html#sqlalchemy.pool.QueuePool).  This pool uses an object equivalent to Python’s
...
this use case, a new flag [create_engine.pool_use_lifo](https://docs.sqlalchemy.org/en/20/core/engines.html#sqlalchemy.create_engine.params.pool_use_lifo) is added
...
[Warnings emitted when coercing full SQL fragments into text()](https://docs.sqlalchemy.org/en/20/changelog/migration_10.html#migration-2992), have now been converted into exceptions.    Continued
concerns have been raised regarding the automatic coercion of string fragments
passed to methods like [Query.filter()](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query.filter) and [Select.order_by()](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.Select.order_by) being
converted to [text()](https://docs.sqlalchemy.org/en/20/core/sqlelement.html#sqlalchemy.sql.expression.text) constructs, even though this has emitted a warning.
In the case of [Select.order_by()](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.Select.order_by), [Query.order_by()](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query.order_by),
[Select.group_by()](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.Select.group_by), and [Query.group_by()](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query.group_by), a string label or column
name is still resolved into the corresponding expression construct, however if
the resolution fails, a [CompileError](https://docs.sqlalchemy.org/en/20/core/exceptions.html#sqlalchemy.exc.CompileError) is raised, thus preventing raw
...
connection pool parameter [Pool.use_threadlocal](https://docs.sqlalchemy.org/en/20/core/pooling.html#sqlalchemy.pool.Pool.params.use_threadlocal) is also deprecated
as it does not actually have any effect in most cases, as is the
`Engine.contextual_connect()` method, which is normally synonymous with
the [Engine.connect()](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Engine.connect) method except in the case where the threadlocal
engine is in use.
...
The parameters [String.convert_unicode](https://docs.sqlalchemy.org/en/20/core/type_basics.html#sqlalchemy.types.String.params.convert_unicode) and
[create_engine.convert_unicode](https://docs.sqlalchemy.org/en/20/core/engines.html#sqlalchemy.create_engine.params.convert_unicode) are deprecated.    The purpose of
...
back from [Inspector.get_table_names()](https://docs.sqlalchemy.org/en/20/core/reflection.html#sqlalchemy.engine.reflection.Inspector.get_table_names) and additionally the columns
...
pool pre-ping feature, described at [Disconnect Handling - Pessimistic](https://docs.sqlalchemy.org/en/20/core/pooling.html#pool-disconnects-pessimistic).
...
A new datatype [JSON](https://docs.sqlalchemy.org/en/20/dialects/sqlite.html#sqlalchemy.dialects.sqlite.JSON) is added which implements SQLite’s json
member access functions on behalf of the [JSON](https://docs.sqlalchemy.org/en/20/core/type_basics.html#sqlalchemy.types.JSON)
...
in SQLite’s own documentation at [https://www.sqlite.org/json1.html](https://www.sqlite.org/json1.html) the name
...
keyword added to objects like [UniqueConstraint](https://docs.sqlalchemy.org/en/20/core/constraints.html#sqlalchemy.schema.UniqueConstraint)  as well
as several [Column](https://docs.sqlalchemy.org/en/20/core/metadata.html#sqlalchemy.schema.Column) -specific variants:
...
[ON CONFLICT support for constraints](https://docs.sqlalchemy.org/en/20/dialects/sqlite.html#sqlite-on-conflict-ddl)
...
The [Unicode](https://docs.sqlalchemy.org/en/20/core/type_basics.html#sqlalchemy.types.Unicode) and [UnicodeText](https://docs.sqlalchemy.org/en/20/core/type_basics.html#sqlalchemy.types.UnicodeText) datatypes by default now
...
`setinputsizes()` when bound parameters using [Unicode](https://docs.sqlalchemy.org/en/20/core/type_basics.html#sqlalchemy.types.Unicode) or
[UnicodeText](https://docs.sqlalchemy.org/en/20/core/type_basics.html#sqlalchemy.types.UnicodeText) are used; cx_Oracle handles the string value natively.
...
the flag `use_nchar_for_unicode` can be passed to [create_engine()](https://docs.sqlalchemy.org/en/20/core/engines.html#sqlalchemy.create_engine) to
re-enable the old behavior.

As always, using the [NVARCHAR2](https://docs.sqlalchemy.org/en/20/dialects/oracle.html#sqlalchemy.dialects.oracle.NVARCHAR2) and [NCLOB](https://docs.sqlalchemy.org/en/20/dialects/oracle.html#sqlalchemy.dialects.oracle.NCLOB)
...
[Unicode](https://docs.sqlalchemy.org/en/20/core/type_basics.html#sqlalchemy.types.Unicode) or Oracle’s NVARCHAR2/NCHAR/NCLOB datatypes.
...
- The value of the `threaded` parameter, which has always been defaulted
  ...
  The SQLAlchemy [Connection](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Connection) object is not considered to be thread-safe
  itself so there’s no need for this flag to be passed.
- It’s deprecated to pass `threaded` to [create_engine()](https://docs.sqlalchemy.org/en/20/core/engines.html#sqlalchemy.create_engine) itself.
  To set the value of `threaded` to `True`, pass it to either the
  [create_engine.connect_args](https://docs.sqlalchemy.org/en/20/core/engines.html#sqlalchemy.create_engine.params.connect_args) dictionary or use the query
...
- As was the case earlier, all cx_Oracle `.connect()` arguments are accepted
  via the [create_engine.connect_args](https://docs.sqlalchemy.org/en/20/core/engines.html#sqlalchemy.create_engine.params.connect_args) dictionary, the documentation
...
Pass it via [create_engine()](https://docs.sqlalchemy.org/en/20/core/engines.html#sqlalchemy.create_engine):
...
[Fast Executemany Mode](https://docs.sqlalchemy.org/en/20/dialects/mssql.html#mssql-pyodbc-fastexecutemany)
...
`CREATE SEQUENCE` syntax.  In [#4235](https://www.sqlalchemy.org/trac/ticket/4235), SQLAlchemy will add support for
these using [Sequence](https://docs.sqlalchemy.org/en/20/core/defaults.html#sqlalchemy.schema.Sequence) in the same way as for any other dialect.
However, the current situation is that [Sequence](https://docs.sqlalchemy.org/en/20/core/defaults.html#sqlalchemy.schema.Sequence) has been repurposed
...
using [Sequence](https://docs.sqlalchemy.org/en/20/core/defaults.html#sqlalchemy.schema.Sequence) will emit a deprecation warning throughout the
...
on [Column](https://docs.sqlalchemy.org/en/20/core/metadata.html#sqlalchemy.schema.Column):
...
but valid SQL Server use case, use the [Column.autoincrement](https://docs.sqlalchemy.org/en/20/core/metadata.html#sqlalchemy.schema.Column.params.autoincrement) flag,
