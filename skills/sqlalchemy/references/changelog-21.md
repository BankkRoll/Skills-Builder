...
A new series of [Session](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session) methods which provide hooks directly
...
[Bulk Operations](https://docs.sqlalchemy.org/en/20/orm/persistence_techniques.html#bulk-operations) - introduction and full documentation

[#3100](https://www.sqlalchemy.org/trac/ticket/3100)

### New Performance Example Suite

Inspired by the benchmarking done for the [Bulk Operations](https://docs.sqlalchemy.org/en/20/orm/persistence_techniques.html#bulk-operations) feature
as well as for the [How can I profile a SQLAlchemy powered application?](https://docs.sqlalchemy.org/en/20/faq/performance.html#faq-how-to-profile) section of the FAQ, a new
...
straightforward construction an invocation of [Query](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query) objects
using caching, which upon successive calls features vastly reduced
Python function call overhead (over 75%).    By  specifying a
[Query](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query) object as a series of lambdas which are only invoked
...
### Improvements to declarative mixins,@declared_attrand related features

The declarative system in conjunction with [declared_attr](https://docs.sqlalchemy.org/en/20/orm/mapping_api.html#sqlalchemy.orm.declared_attr) has been
overhauled to support new capabilities.

A function decorated with [declared_attr](https://docs.sqlalchemy.org/en/20/orm/mapping_api.html#sqlalchemy.orm.declared_attr) is now called only **after**
...
and `SomeClass.foobar` will be the final [Column](https://docs.sqlalchemy.org/en/20/core/metadata.html#sqlalchemy.schema.Column) object that is
...
The [declared_attr](https://docs.sqlalchemy.org/en/20/orm/mapping_api.html#sqlalchemy.orm.declared_attr) function now **memoizes** the value
...
[declared_attr](https://docs.sqlalchemy.org/en/20/orm/mapping_api.html#sqlalchemy.orm.declared_attr) is called upon.
...
the [declared_attr](https://docs.sqlalchemy.org/en/20/orm/mapping_api.html#sqlalchemy.orm.declared_attr) function is called upon from other
[declared_attr](https://docs.sqlalchemy.org/en/20/orm/mapping_api.html#sqlalchemy.orm.declared_attr) functions locally present before the class is
actually mapped.
...
[declared_attr.cascading](https://docs.sqlalchemy.org/en/20/orm/mapping_api.html#sqlalchemy.orm.declared_attr.cascading) is added.  With this modifier, the
...
With [declared_attr.cascading](https://docs.sqlalchemy.org/en/20/orm/mapping_api.html#sqlalchemy.orm.declared_attr.cascading), individual behaviors can be
applied:
...
Finally, the [AbstractConcreteBase](https://docs.sqlalchemy.org/en/20/orm/extensions/declarative/index.html#sqlalchemy.ext.declarative.AbstractConcreteBase) class has been reworked
...
1.0.  The [Query.yield_per()](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query.yield_per) call is always a good idea when batching
...
at once.  Without the [Query.yield_per()](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query.yield_per), the above script on the
...
We took a look into the `KeyedTuple` implementation in the hopes
...
The `KeyedTuple` class is used rather than Python’s
`collections.namedtuple()`, because the latter has a very complex
type-creation routine that benchmarks much slower than `KeyedTuple`.
However, when fetching hundreds of thousands of rows,
`collections.namedtuple()` quickly overtakes `KeyedTuple` which
...
- The mapping does not use a `mapper.version_id_col`, or
...
The [Session.get_bind()](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session.get_bind) method is invoked whenever a query or unit
...
A series of issues were repaired where the [Session.get_bind()](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session.get_bind)
would not receive the primary [Mapper](https://docs.sqlalchemy.org/en/20/orm/mapping_api.html#sqlalchemy.orm.Mapper) of the [Query](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query),
...
a [Query](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query) object).

The [Mapper](https://docs.sqlalchemy.org/en/20/orm/mapping_api.html#sqlalchemy.orm.Mapper) object, when passed to [Session.get_bind()](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session.get_bind),
is typically used by sessions that make use of the
[Session.binds](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session.params.binds) parameter to associate mappers with a
...
[Session.get_bind()](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session.get_bind) method that provides some pattern of
...
- [Query.count()](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query.count):
  ...
  ```
- [Query.update()](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query.update) and [Query.delete()](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query.delete), both for the UPDATE/DELETE
...
The `InspectionAttr.info` collection is now available on every kind
of object that one would retrieve from the [Mapper.all_orm_descriptors](https://docs.sqlalchemy.org/en/20/orm/mapping_api.html#sqlalchemy.orm.Mapper.all_orm_descriptors)
collection.  This includes [hybrid_property](https://docs.sqlalchemy.org/en/20/orm/extensions/hybrid.html#sqlalchemy.ext.hybrid.hybrid_property) and [association_proxy()](https://docs.sqlalchemy.org/en/20/orm/extensions/associationproxy.html#sqlalchemy.ext.associationproxy.association_proxy).
...
[Mapper.all_orm_descriptors](https://docs.sqlalchemy.org/en/20/orm/mapping_api.html#sqlalchemy.orm.Mapper.all_orm_descriptors) namespace:
...
It is also available as a constructor argument for all [SchemaItem](https://docs.sqlalchemy.org/en/20/core/metadata.html#sqlalchemy.schema.SchemaItem)
objects (e.g. [ForeignKey](https://docs.sqlalchemy.org/en/20/core/constraints.html#sqlalchemy.schema.ForeignKey), [UniqueConstraint](https://docs.sqlalchemy.org/en/20/core/constraints.html#sqlalchemy.schema.UniqueConstraint) etc.) as well
as remaining ORM constructs such as [synonym()](https://docs.sqlalchemy.org/en/20/orm/mapped_attributes.html#sqlalchemy.orm.synonym).
...
A variety of issues regarding [column_property()](https://docs.sqlalchemy.org/en/20/orm/mapping_api.html#sqlalchemy.orm.column_property) have been fixed,
most specifically with regards to the [aliased()](https://docs.sqlalchemy.org/en/20/orm/queryguide/api.html#sqlalchemy.orm.aliased) construct as well
as the “order by label” logic introduced in 0.9 (see [Label constructs can now render as their name alone in an ORDER BY](https://docs.sqlalchemy.org/en/20/changelog/migration_09.html#migration-1068)).
...
The [Select.limit()](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.Select.limit) and [Select.offset()](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.Select.offset) methods now accept
any SQL expression, in addition to integer values, as arguments.  The ORM
[Query](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query) object also passes through any expression to the underlying
[Select](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.Select) object.   Typically
...
instead raise a [CompileError](https://docs.sqlalchemy.org/en/20/core/exceptions.html#sqlalchemy.exc.CompileError) on access.  A third-party dialect which
...
### Theuse_alterflag onForeignKeyConstraintis (usually) no longer needed

The [MetaData.create_all()](https://docs.sqlalchemy.org/en/20/core/metadata.html#sqlalchemy.schema.MetaData.create_all) and [MetaData.drop_all()](https://docs.sqlalchemy.org/en/20/core/metadata.html#sqlalchemy.schema.MetaData.drop_all) methods will
...
need to specify [ForeignKeyConstraint.use_alter](https://docs.sqlalchemy.org/en/20/core/constraints.html#sqlalchemy.schema.ForeignKeyConstraint.params.use_alter).   Additionally,
...
The [ForeignKeyConstraint.use_alter](https://docs.sqlalchemy.org/en/20/core/constraints.html#sqlalchemy.schema.ForeignKeyConstraint.params.use_alter) and
[ForeignKey.use_alter](https://docs.sqlalchemy.org/en/20/core/constraints.html#sqlalchemy.schema.ForeignKey.params.use_alter) flags remain in place, and continue to have
...
[ForeignKey](https://docs.sqlalchemy.org/en/20/core/constraints.html#sqlalchemy.schema.ForeignKey) and [ForeignKeyConstraint](https://docs.sqlalchemy.org/en/20/core/constraints.html#sqlalchemy.schema.ForeignKeyConstraint) objects which should
be explicitly omitted from the sort.

See also

[Creating/Dropping Foreign Key Constraints via ALTER](https://docs.sqlalchemy.org/en/20/core/constraints.html#use-alter) - full description of the new behavior.

[#3282](https://www.sqlalchemy.org/trac/ticket/3282)

### ResultProxy “auto close” is now a “soft” close

For many releases, the `ResultProxy` object has always been
...
upon `ResultProxy.close()` explicitly; as all DBAPI resources had been
...
`ResultProxy.fetchone()`, `ResultProxy.fetchmany()` or
`ResultProxy.fetchall()` would now raise a [ResourceClosedError](https://docs.sqlalchemy.org/en/20/core/exceptions.html#sqlalchemy.exc.ResourceClosedError):
...
result proxy, such as the `BufferedColumnResultProxy` used by the
cx_oracle dialect for certain datatypes.

To solve this, the “closed” state of the `ResultProxy` has been
...
establishing the fetch methods as “closed”.   The `ResultProxy.close()`
method is now never called implicitly, only the `ResultProxy._soft_close()`
...
### CHECK Constraints now support the%(column_0_name)stoken in naming conventions

The `%(column_0_name)s` will derive from the first column found in the
expression of a [CheckConstraint](https://docs.sqlalchemy.org/en/20/core/constraints.html#sqlalchemy.schema.CheckConstraint):
...
[SchemaType](https://docs.sqlalchemy.org/en/20/core/type_basics.html#sqlalchemy.types.SchemaType) such as [Boolean](https://docs.sqlalchemy.org/en/20/core/type_basics.html#sqlalchemy.types.Boolean) or [Enum](https://docs.sqlalchemy.org/en/20/core/type_basics.html#sqlalchemy.types.Enum) will also
...
Since at least version 0.8, a [Constraint](https://docs.sqlalchemy.org/en/20/core/constraints.html#sqlalchemy.schema.Constraint) has had the ability to
“auto-attach” itself to a [Table](https://docs.sqlalchemy.org/en/20/core/metadata.html#sqlalchemy.schema.Table) based on being passed table-attached columns:
...
this same auto-attachment logic can now function even if the [Column](https://docs.sqlalchemy.org/en/20/core/metadata.html#sqlalchemy.schema.Column)
objects are not yet associated with the [Table](https://docs.sqlalchemy.org/en/20/core/metadata.html#sqlalchemy.schema.Table); additional events
are established such that when those [Column](https://docs.sqlalchemy.org/en/20/core/metadata.html#sqlalchemy.schema.Column) objects are associated,
the [Constraint](https://docs.sqlalchemy.org/en/20/core/constraints.html#sqlalchemy.schema.Constraint) is also added:
...
version 1.0.4 for [#3411](https://www.sqlalchemy.org/trac/ticket/3411) ensures that this logic
does not occur if the [Constraint](https://docs.sqlalchemy.org/en/20/core/constraints.html#sqlalchemy.schema.Constraint) refers to a mixture of
[Column](https://docs.sqlalchemy.org/en/20/core/metadata.html#sqlalchemy.schema.Column) objects and string column names; as we do not yet have
tracking for the addition of names to a [Table](https://docs.sqlalchemy.org/en/20/core/metadata.html#sqlalchemy.schema.Table):
...
column “b” is attached (as “a” is stated in the [Table](https://docs.sqlalchemy.org/en/20/core/metadata.html#sqlalchemy.schema.Table) constructor
...
The original auto-attach logic of course remains in place, if the [Table](https://docs.sqlalchemy.org/en/20/core/metadata.html#sqlalchemy.schema.Table)
already contains all the target [Column](https://docs.sqlalchemy.org/en/20/core/metadata.html#sqlalchemy.schema.Column) objects at the time
the [Constraint](https://docs.sqlalchemy.org/en/20/core/constraints.html#sqlalchemy.schema.Constraint) is constructed:
...
[Insert.from_select()](https://docs.sqlalchemy.org/en/20/core/dml.html#sqlalchemy.sql.expression.Insert.from_select) now includes Python and SQL-expression defaults if
...
[Insert.from_select.include_defaults](https://docs.sqlalchemy.org/en/20/core/dml.html#sqlalchemy.sql.expression.Insert.from_select.params.include_defaults).

### Column server defaults now render literal values

The “literal binds” compiler flag is switched on when a
[DefaultClause](https://docs.sqlalchemy.org/en/20/core/defaults.html#sqlalchemy.schema.DefaultClause), set up by [Column.server_default](https://docs.sqlalchemy.org/en/20/core/metadata.html#sqlalchemy.schema.Column.params.server_default)
...
A [Table](https://docs.sqlalchemy.org/en/20/core/metadata.html#sqlalchemy.schema.Table) object populated using `autoload=True` will now
include [UniqueConstraint](https://docs.sqlalchemy.org/en/20/core/constraints.html#sqlalchemy.schema.UniqueConstraint) constructs as well as
[Index](https://docs.sqlalchemy.org/en/20/core/constraints.html#sqlalchemy.schema.Index) constructs.  This logic has a few caveats for
...
constraint as well. The [Inspector.get_indexes()](https://docs.sqlalchemy.org/en/20/core/reflection.html#sqlalchemy.engine.reflection.Inspector.get_indexes) and the
[Inspector.get_unique_constraints()](https://docs.sqlalchemy.org/en/20/core/reflection.html#sqlalchemy.engine.reflection.Inspector.get_unique_constraints) methods will continue to
**both** return these entries distinctly, where
[Inspector.get_indexes()](https://docs.sqlalchemy.org/en/20/core/reflection.html#sqlalchemy.engine.reflection.Inspector.get_indexes) now features a token
...
[Index](https://docs.sqlalchemy.org/en/20/core/constraints.html#sqlalchemy.schema.Index) construct is detected as being linked to the
[UniqueConstraint](https://docs.sqlalchemy.org/en/20/core/constraints.html#sqlalchemy.schema.UniqueConstraint), and is **not** present within the
[Table.indexes](https://docs.sqlalchemy.org/en/20/core/metadata.html#sqlalchemy.schema.Table.indexes) collection; only the [UniqueConstraint](https://docs.sqlalchemy.org/en/20/core/constraints.html#sqlalchemy.schema.UniqueConstraint)
will be present in the [Table.constraints](https://docs.sqlalchemy.org/en/20/core/metadata.html#sqlalchemy.schema.Table.constraints) collection.   This
...
and the [Inspector.get_unique_constraints()](https://docs.sqlalchemy.org/en/20/core/reflection.html#sqlalchemy.engine.reflection.Inspector.get_unique_constraints) methods will continue to
**both** return an entry for a UNIQUE index in MySQL,
where [Inspector.get_unique_constraints()](https://docs.sqlalchemy.org/en/20/core/reflection.html#sqlalchemy.engine.reflection.Inspector.get_unique_constraints) features a new token
...
[PostgreSQL Index Reflection](https://docs.sqlalchemy.org/en/20/dialects/postgresql.html#postgresql-index-reflection)

[MySQL / MariaDB Unique Constraints and Reflection](https://docs.sqlalchemy.org/en/20/dialects/mysql.html#mysql-unique-constraints)
...
The documentation for [Query.update()](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query.update) states that the given
...
a call to [Query.update()](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query.update) that was passed strings would have to
...
behavior (now captured in [#3373](https://www.sqlalchemy.org/trac/ticket/3373)) in a future release.
...
it only became apparent as a result of [#3371](https://www.sqlalchemy.org/trac/ticket/3371).
...
for [get_history()](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.attributes.get_history) and related functions.
...
However, as [#3060](https://www.sqlalchemy.org/trac/ticket/3060) (described here in [Priority of attribute changes on relationship-bound attributes vs. FK-bound may appear to change](#migration-3060))
...
value `NEVER_SET`, which is a value that may be sent
to an attribute listener now.   This symbol may also be received when
calling on mapper utility functions such as [Mapper.primary_key_from_instance()](https://docs.sqlalchemy.org/en/20/orm/mapping_api.html#sqlalchemy.orm.Mapper.primary_key_from_instance);
if the primary key attributes have no setting at all, whereas the value
would be `None` before, it will now be the `NEVER_SET`
...
As a side effect of [#3060](https://www.sqlalchemy.org/trac/ticket/3060), setting a relationship-bound attribute to `None`
...
The behavior of [Session.expunge()](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session.expunge) had a bug that caused an
inconsistency in behavior regarding deleted objects.  The
[object_session()](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.object_session) function as well as the [InstanceState.session](https://docs.sqlalchemy.org/en/20/orm/internals.html#sqlalchemy.orm.InstanceState.session)
attribute would still report object as belonging to the [Session](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session)
...
is ongoing subsequent to the delete operation and [Session.expunge()](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session.expunge)
...
that rely on [Session.expunge()](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session.expunge) such as [make_transient()](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.make_transient).

[#3139](https://www.sqlalchemy.org/trac/ticket/3139)

### Joined/Subquery eager loading explicitly disallowed with yield_per

In order to make the [Query.yield_per()](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query.yield_per) method easier to use,
...
When this error is raised, the [lazyload()](https://docs.sqlalchemy.org/en/20/orm/queryguide/relationships.html#sqlalchemy.orm.lazyload) option can be sent with
an asterisk:
...
or use [Query.enable_eagerloads()](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query.enable_eagerloads):

```
q = sess.query(Object).enable_eagerloads(False).yield_per(100)
```

The [lazyload()](https://docs.sqlalchemy.org/en/20/orm/queryguide/relationships.html#sqlalchemy.orm.lazyload) option has the advantage that additional many-to-one
...
That is, the `A.bs` is part of a “path”.  As part of [#3367](https://www.sqlalchemy.org/trac/ticket/3367),
...
[undefer()](https://docs.sqlalchemy.org/en/20/orm/queryguide/columns.html#sqlalchemy.orm.undefer) or similar option should now be used, in order
...
The new [Bundle](https://docs.sqlalchemy.org/en/20/orm/queryguide/api.html#sqlalchemy.orm.Bundle) object of 0.9 has a small change in API,
...
As introduced in [Right-nested inner joins available in joined eager loads](https://docs.sqlalchemy.org/en/20/changelog/migration_09.html#feature-2976) from version 0.9, the behavior of
...
than using an OUTER join, and allows the [joinedload.innerjoin](https://docs.sqlalchemy.org/en/20/orm/queryguide/relationships.html#sqlalchemy.orm.joinedload.params.innerjoin)
...
[Right-nested inner joins available in joined eager loads](https://docs.sqlalchemy.org/en/20/changelog/migration_09.html#feature-2976) - description of the feature as introduced in 0.9.4.
...
June 9, 2015) when the [Query.update()](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query.update) or [Query.delete()](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query.delete) methods
are invoked against a query which has also called upon [Query.join()](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query.join),
[Query.outerjoin()](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query.outerjoin),
[Query.select_from()](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query.select_from) or `Query.from_self()`.  These are unsupported
...
### query.update() withsynchronize_session='evaluate'raises on multi-table update

The “evaluator” for [Query.update()](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query.update) won’t work with multi-table
...
Using `Query.from_self()` or [Query.count()](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query.count) against a subclass
...
Above, because we joined to a subclass `FooWidget`, [Query.join()](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query.join)
...
full SQL string to [Connection.execute()](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Connection.execute), but that you can send strings
with SQL expressions into many functions, such as [Select.where()](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.Select.where),
[Query.filter()](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query.filter), and [Select.order_by()](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.Select.order_by).
...
using a [select()](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.select) construct where virtually all components of it
...
like [Query.filter()](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query.filter).
...
The warnings make use of the [Session.get_bind() handles a wider variety of inheritance scenarios](#feature-3178) so that parameterized warnings
can be emitted safely without running out of memory, and as always, if
one wishes the warnings to be exceptions, the
[Python Warnings Filter](https://docs.python.org/2/library/warnings.html)
should be used:
...
about the text if we use [column()](https://docs.sqlalchemy.org/en/20/core/sqlelement.html#sqlalchemy.sql.expression.column) and [table()](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.table):
...
Where note also that [table()](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.table) and [column()](https://docs.sqlalchemy.org/en/20/core/sqlelement.html#sqlalchemy.sql.expression.column) can now
be imported from “sqlalchemy” without the “sql” part.

The behavior here applies to [select()](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.select) as well as to key methods
on [Query](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query), including [Query.filter()](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query.filter),
[Query.from_statement()](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query.from_statement) and [Query.having()](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query.having).
...
[select()](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.select) or [Query](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query) that refers to some column name or named
...
a [text()](https://docs.sqlalchemy.org/en/20/core/sqlelement.html#sqlalchemy.sql.expression.text) construct, or a column, table, etc., and if we use it as a
...
be qualified with [text()](https://docs.sqlalchemy.org/en/20/core/sqlelement.html#sqlalchemy.sql.expression.text) or similar.
...
version of [Insert.values()](https://docs.sqlalchemy.org/en/20/core/dml.html#sqlalchemy.sql.expression.Insert.values) were essentially not implemented, and
...
### The INSERT…FROM SELECT construct now impliesinline=True
...
### autoload_withnow impliesautoload=True

A [Table](https://docs.sqlalchemy.org/en/20/core/metadata.html#sqlalchemy.schema.Table) can be set up for reflection by passing
[Table.autoload_with](https://docs.sqlalchemy.org/en/20/core/metadata.html#sqlalchemy.schema.Table.params.autoload_with) alone:
...
case where a [Connection](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Connection) object was invalidated, and then tried
to reconnect and encountered an error; this has been resolved.

Additionally, the recently added `ConnectionEvents.handle_error()`
event is now invoked for errors that occur upon initial connect, upon
reconnect, and when [create_engine()](https://docs.sqlalchemy.org/en/20/core/engines.html#sqlalchemy.create_engine) is used given a custom connection
function via [create_engine.creator](https://docs.sqlalchemy.org/en/20/core/engines.html#sqlalchemy.create_engine.params.creator).

The [ExceptionContext](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.ExceptionContext) object has a new datamember
[ExceptionContext.engine](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.ExceptionContext.engine) that will always refer to the [Engine](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Engine)
in use, in those cases when the [Connection](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Connection) object is not available
...
[ForeignKeyConstraint.columns](https://docs.sqlalchemy.org/en/20/core/constraints.html#sqlalchemy.schema.ForeignKeyConstraint.columns) was previously a plain list
containing either strings or [Column](https://docs.sqlalchemy.org/en/20/core/metadata.html#sqlalchemy.schema.Column) objects, depending on
how the [ForeignKeyConstraint](https://docs.sqlalchemy.org/en/20/core/constraints.html#sqlalchemy.schema.ForeignKeyConstraint) was constructed and whether it was
associated with a table.  The collection is now a [ColumnCollection](https://docs.sqlalchemy.org/en/20/core/sqlelement.html#sqlalchemy.sql.expression.ColumnCollection),
and is only initialized after the [ForeignKeyConstraint](https://docs.sqlalchemy.org/en/20/core/constraints.html#sqlalchemy.schema.ForeignKeyConstraint) is
associated with a [Table](https://docs.sqlalchemy.org/en/20/core/metadata.html#sqlalchemy.schema.Table).  A new accessor
[ForeignKeyConstraint.column_keys](https://docs.sqlalchemy.org/en/20/core/constraints.html#sqlalchemy.schema.ForeignKeyConstraint.column_keys)
...
The sorting of tables resulting from the [MetaData.sorted_tables](https://docs.sqlalchemy.org/en/20/core/metadata.html#sqlalchemy.schema.MetaData.sorted_tables)
...
when emitting [MetaData.create_all()](https://docs.sqlalchemy.org/en/20/core/metadata.html#sqlalchemy.schema.MetaData.create_all) or [MetaData.drop_all()](https://docs.sqlalchemy.org/en/20/core/metadata.html#sqlalchemy.schema.MetaData.drop_all).
...
rendering only `SELECT NULL AS anon_1`, because the two [null()](https://docs.sqlalchemy.org/en/20/core/sqlelement.html#sqlalchemy.sql.expression.null)
...
The [Inspector.get_table_names()](https://docs.sqlalchemy.org/en/20/core/reflection.html#sqlalchemy.engine.reflection.Inspector.get_table_names) and [Inspector.get_view_names()](https://docs.sqlalchemy.org/en/20/core/reflection.html#sqlalchemy.engine.reflection.Inspector.get_view_names)
...
out to two new methods [Inspector.get_temp_table_names()](https://docs.sqlalchemy.org/en/20/core/reflection.html#sqlalchemy.engine.reflection.Inspector.get_temp_table_names) and
[Inspector.get_temp_view_names()](https://docs.sqlalchemy.org/en/20/core/reflection.html#sqlalchemy.engine.reflection.Inspector.get_temp_view_names).
...
[Inspector.get_columns()](https://docs.sqlalchemy.org/en/20/core/reflection.html#sqlalchemy.engine.reflection.Inspector.get_columns) continues to function for most if not all
dialects.   For SQLite specifically, there is a bug fix for UNIQUE constraint
reflection from temp tables as well, which is [#3203](https://www.sqlalchemy.org/trac/ticket/3203).
...
The rules for PostgreSQL [ENUM](https://docs.sqlalchemy.org/en/20/dialects/postgresql.html#sqlalchemy.dialects.postgresql.ENUM) have been made more strict
with regards to creating and dropping of the TYPE.

An [ENUM](https://docs.sqlalchemy.org/en/20/dialects/postgresql.html#sqlalchemy.dialects.postgresql.ENUM) that is created **without** being explicitly
associated with a [MetaData](https://docs.sqlalchemy.org/en/20/core/metadata.html#sqlalchemy.schema.MetaData) object will be created *and* dropped
corresponding to [Table.create()](https://docs.sqlalchemy.org/en/20/core/metadata.html#sqlalchemy.schema.Table.create) and [Table.drop()](https://docs.sqlalchemy.org/en/20/core/metadata.html#sqlalchemy.schema.Table.drop):
...
An [ENUM](https://docs.sqlalchemy.org/en/20/dialects/postgresql.html#sqlalchemy.dialects.postgresql.ENUM) that is created **with** being explicitly
associated with a [MetaData](https://docs.sqlalchemy.org/en/20/core/metadata.html#sqlalchemy.schema.MetaData) object will *not* be created *or* dropped
corresponding to [Table.create()](https://docs.sqlalchemy.org/en/20/core/metadata.html#sqlalchemy.schema.Table.create) and [Table.drop()](https://docs.sqlalchemy.org/en/20/core/metadata.html#sqlalchemy.schema.Table.drop), with
the exception of [Table.create()](https://docs.sqlalchemy.org/en/20/core/metadata.html#sqlalchemy.schema.Table.create) called with the `checkfirst=True`
flag:
...
the [Table](https://docs.sqlalchemy.org/en/20/core/metadata.html#sqlalchemy.schema.Table) construct.
...
The [inspect()](https://docs.sqlalchemy.org/en/20/core/inspection.html#sqlalchemy.inspect) method returns a [PGInspector](https://docs.sqlalchemy.org/en/20/dialects/postgresql.html#sqlalchemy.dialects.postgresql.base.PGInspector) object in the
case of PostgreSQL, which includes a new [PGInspector.get_enums()](https://docs.sqlalchemy.org/en/20/dialects/postgresql.html#sqlalchemy.dialects.postgresql.base.PGInspector.get_enums)
...
[PGInspector.get_enums()](https://docs.sqlalchemy.org/en/20/dialects/postgresql.html#sqlalchemy.dialects.postgresql.base.PGInspector.get_enums)
...
- [Inspector.get_view_names()](https://docs.sqlalchemy.org/en/20/core/reflection.html#sqlalchemy.engine.reflection.Inspector.get_view_names) will return plain and materialized view
  names.
- [Inspector.get_table_names()](https://docs.sqlalchemy.org/en/20/core/reflection.html#sqlalchemy.engine.reflection.Inspector.get_table_names) does **not** change for PostgreSQL, it
  continues to return only the names of plain tables.
- A new method [PGInspector.get_foreign_table_names()](https://docs.sqlalchemy.org/en/20/dialects/postgresql.html#sqlalchemy.dialects.postgresql.base.PGInspector.get_foreign_table_names) is added which
...
### PostgreSQLhas_table()now works for temporary tables
...
The [create_engine.encoding](https://docs.sqlalchemy.org/en/20/core/engines.html#sqlalchemy.create_engine.params.encoding) parameter is now honored
...
[explicit_defaults_for_timestamp](https://dev.mysql.com/doc/refman/5.6/en/server-system-variables.html#sysvar_explicit_defaults_for_timestamp) which repairs MySQL’s non-standard
...
The [SET](https://docs.sqlalchemy.org/en/20/dialects/mysql.html#sqlalchemy.dialects.mysql.SET) type historically not included a system of handling
...
case, and the default behavior of [SET](https://docs.sqlalchemy.org/en/20/dialects/mysql.html#sqlalchemy.dialects.mysql.SET) is now:
...
- To handle the case of a set type that actually wishes includes the blank
  ...
  flag [SET.retrieve_as_bitwise](https://docs.sqlalchemy.org/en/20/dialects/mysql.html#sqlalchemy.dialects.mysql.SET.params.retrieve_as_bitwise) is added in order to
  enable this.

Using the [SET.retrieve_as_bitwise](https://docs.sqlalchemy.org/en/20/dialects/mysql.html#sqlalchemy.dialects.mysql.SET.params.retrieve_as_bitwise) flag allows the set
...
The default behavior of [SET](https://docs.sqlalchemy.org/en/20/dialects/mysql.html#sqlalchemy.dialects.mysql.SET) otherwise remains the same,
...
### Changed the default value ofraise_on_warningsfor MySQL-Connector
...
[#2682](https://www.sqlalchemy.org/trac/ticket/2682) disallowed the MySQL dialect from making use of the
...
but the [true()](https://docs.sqlalchemy.org/en/20/core/sqlelement.html#sqlalchemy.sql.expression.true) and [false()](https://docs.sqlalchemy.org/en/20/core/sqlelement.html#sqlalchemy.sql.expression.false) symbols again produce the
...
The return type of a [ColumnOperators.match()](https://docs.sqlalchemy.org/en/20/core/sqlelement.html#sqlalchemy.sql.expression.ColumnOperators.match) expression is now a new type
called [MatchType](https://docs.sqlalchemy.org/en/20/core/type_basics.html#sqlalchemy.types.MatchType).  This is a subclass of [Boolean](https://docs.sqlalchemy.org/en/20/core/type_basics.html#sqlalchemy.types.Boolean),
...
The dialect for [Drizzle](https://en.wikipedia.org/wiki/Drizzle_(database_server)) is now an external
dialect, available at [https://bitbucket.org/zzzeek/sqlalchemy-drizzle](https://bitbucket.org/zzzeek/sqlalchemy-drizzle).
...
The rendering of the [TextClause](https://docs.sqlalchemy.org/en/20/core/sqlelement.html#sqlalchemy.sql.expression.TextClause), [UnicodeText](https://docs.sqlalchemy.org/en/20/core/type_basics.html#sqlalchemy.types.UnicodeText), and [LargeBinary](https://docs.sqlalchemy.org/en/20/core/type_basics.html#sqlalchemy.types.LargeBinary)
...
Microsoft.  See [Large Text/Binary Type Deprecation](https://docs.sqlalchemy.org/en/20/dialects/mssql.html#mssql-large-type-deprecation) for details.
...
`CTE.with_suffixes()` that can assist with Oracle’s special directives:
...
[Oracle Database Table Options](https://docs.sqlalchemy.org/en/20/dialects/oracle.html#oracle-table-options)

[Oracle Database Specific Index Options](https://docs.sqlalchemy.org/en/20/dialects/oracle.html#oracle-index-options)
