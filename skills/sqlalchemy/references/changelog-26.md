...
- [What’s New in SQLAlchemy 2.0?](https://docs.sqlalchemy.org/en/20/changelog/whatsnew_20.html) - new features and behaviors for SQLAlchemy 2.0
...
[What’s New in SQLAlchemy 2.0?](https://docs.sqlalchemy.org/en/20/changelog/whatsnew_20.html) for an overview of new features and
capabilities.
...
[Query](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query)  and Declarative. It also hopes standardize some
...
- [New ORM statement paradigm](https://docs.sqlalchemy.org/en/20/changelog/migration_14.html#change-5159)
- [SQL caching throughout Core and ORM](https://docs.sqlalchemy.org/en/20/changelog/migration_14.html#change-4639)
- [New Declarative features, ORM integration](https://docs.sqlalchemy.org/en/20/changelog/migration_14.html#change-5508)
- [New Result object](https://docs.sqlalchemy.org/en/20/changelog/migration_14.html#change-result-14-core)
- [select() / case() Accept Positional Expressions](https://docs.sqlalchemy.org/en/20/changelog/migration_14.html#change-5284)
- [asyncio support for Core and ORM](https://docs.sqlalchemy.org/en/20/changelog/migration_14.html#change-3414)

The above bullets link to the description of these new paradigms as introduced
in SQLAlchemy 1.4. in the [What’s New in SQLAlchemy 1.4?](https://docs.sqlalchemy.org/en/20/changelog/migration_14.html) document.

For SQLAlchemy 2.0, all API features and behaviors
that were marked as [deprecated for 2.0](https://docs.sqlalchemy.org/en/20/changelog/migration_14.html#deprecation-20-mode) are
now finalized; in particular, major APIs that are **no longer present**
include:

- [Bound MetaData and connectionless execution](#migration-20-implicit-execution)
- [Emulated autocommit on Connection](#migration-20-autocommit)
- [The Session.autocommit parameter / mode](#migration-20-session-autocommit)
- [List / keyword arguments to select()](#migration-20-5284)
- Python 2 support
...
[1.x -> 2.x Migration Overview](#migration-20-overview).
...
[relationship.viewonly](https://docs.sqlalchemy.org/en/20/orm/relationship_api.html#sqlalchemy.orm.relationship.params.viewonly) and
[relationship.sync_backref](https://docs.sqlalchemy.org/en/20/orm/relationship_api.html#sqlalchemy.orm.relationship.params.sync_backref) flags.
...
warnings; these are warnings emitted for the [SADeprecationWarning](https://docs.sqlalchemy.org/en/20/core/exceptions.html#sqlalchemy.exc.SADeprecationWarning)
class.
...
- [The URL object is now immutable](https://docs.sqlalchemy.org/en/20/changelog/migration_14.html#change-5526) - this impacts code that would be manipulating the
  [URL](https://docs.sqlalchemy.org/en/20/core/engines.html#sqlalchemy.engine.URL) object and may impact code that makes use of the
  [CreateEnginePlugin](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.CreateEnginePlugin) extension point.   This is an uncommon
  ...
  the relatively new and little-known [CreateEnginePlugin](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.CreateEnginePlugin)
  class found two projects that were unaffected by the change.
- [A SELECT statement is no longer implicitly considered to be a FROM clause](https://docs.sqlalchemy.org/en/20/changelog/migration_14.html#change-4617) - this change may impact code that was somehow relying
  upon behavior that was mostly unusable in the [Select](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.Select) construct,
  ...
  relying upon this.
- [select().join() and outerjoin() add JOIN criteria to the current query, rather than creating a subquery](https://docs.sqlalchemy.org/en/20/changelog/migration_14.html#change-select-join) - somewhat related, the [Select](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.Select) class
  featured `.join()` and `.outerjoin()` methods that implicitly created a
  subquery and then returned a [Join](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.Join) construct, which again would
  ...
  where these methods now work the same way as the ORM [Query.join()](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query.join)
  method.
- [Many Core and ORM statement objects now perform much of their construction and validation in the compile phase](https://docs.sqlalchemy.org/en/20/changelog/migration_14.html#change-deferred-construction) - some error messages related to
  construction of a [Query](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query) or [Select](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.Select) may not be
...
[What’s New in SQLAlchemy 1.4?](https://docs.sqlalchemy.org/en/20/changelog/migration_14.html) document.
...
as “legacy”, namely the use of the `Engine.execute()` method
...
variable, and additionally ensure that a [warnings filter](https://docs.python.org/3/library/warnings.html#the-warnings-filter) that will not
...
`RemovedIn20Warning` warnings with “2.0 deprecations mode” turned
...
### Migration to 2.0 Step Four - Use thefutureflag on Engine

The [Engine](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Engine) object features an updated
transaction-level API in version 2.0.  In 1.4, this new API is available
by passing the flag `future=True` to the [create_engine()](https://docs.sqlalchemy.org/en/20/core/engines.html#sqlalchemy.create_engine)
function.

When the [create_engine.future](https://docs.sqlalchemy.org/en/20/core/engines.html#sqlalchemy.create_engine.params.future) flag is used, the [Engine](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Engine)
and [Connection](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Connection) objects support the 2.0 API fully and not at all
any legacy features, including the new argument format for [Connection.execute()](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Connection.execute),
the removal of “implicit autocommit”, string statements require the
[text()](https://docs.sqlalchemy.org/en/20/core/sqlelement.html#sqlalchemy.sql.expression.text) construct unless the [Connection.exec_driver_sql()](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Connection.exec_driver_sql)
method is used, and connectionless execution from the [Engine](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Engine)
is removed.

If all `RemovedIn20Warning` warnings have been resolved regarding
use of the [Engine](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Engine) and [Connection](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Connection), then the
[create_engine.future](https://docs.sqlalchemy.org/en/20/core/engines.html#sqlalchemy.create_engine.params.future) flag may be enabled and there should be
no errors raised.

The new engine is described at [Engine](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Engine) which delivers a new
[Connection](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Connection) object.    In addition to the above changes, the,
[Connection](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Connection) object features
[Connection.commit()](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Connection.commit) and
[Connection.rollback()](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Connection.rollback) methods, to support the new
...
### Migration to 2.0 Step Five - Use thefutureflag on Session

The [Session](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session) object also features an updated transaction/connection
level API in version 2.0.  This API is available in 1.4 using the
[Session.future](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session.params.future) flag on [Session](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session) or on
[sessionmaker](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.sessionmaker).

The [Session](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session) object supports “future” mode in place, and involves
these changes:

1. The [Session](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session) no longer supports “bound metadata” when it
  resolves the engine to be used for connectivity.   This means that an
  [Engine](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Engine) object **must** be passed to the constructor (this
  may be either a legacy or future style object).
2. The [Session.begin.subtransactions](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session.begin.params.subtransactions) flag is no longer
  supported.
3. The [Session.commit()](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session.commit) method always emits a COMMIT to the database,
  rather than attempting to reconcile “subtransactions”.
4. The [Session.rollback()](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session.rollback) method always rolls back the full
  stack of transactions at once, rather than attempting to keep
  “subtransactions” in place.

The [Session](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session) also supports more flexible creational patterns
in 1.4 which are now closely matched to the patterns used by the
[Connection](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Connection) object.   Highlights include that the
[Session](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session) may be used as a context manager:
...
In addition, the [sessionmaker](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.sessionmaker) object supports a
[sessionmaker.begin()](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.sessionmaker.begin) context manager that will create a
[Session](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session) and begin /commit a transaction in one block:
...
See the section [Session-level vs. Engine level transaction control](https://docs.sqlalchemy.org/en/20/orm/session_transaction.html#orm-session-vs-engine) for a comparison of
[Session](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session) creational patterns compared to those of
...
### Migration to 2.0 Step Six - Add__allow_unmapped__to explicitly typed ORM models

SQLAlchemy 2.0 has new support for runtime interpretation of [PEP 484](https://peps.python.org/pep-0484/) typing annotations
on ORM models.   A requirement of these annotations is that they must make use
of the [Mapped](https://docs.sqlalchemy.org/en/20/orm/internals.html#sqlalchemy.orm.Mapped) generic container.  Annotations which don’t use
[Mapped](https://docs.sqlalchemy.org/en/20/orm/internals.html#sqlalchemy.orm.Mapped) which link to constructs such as [relationship()](https://docs.sqlalchemy.org/en/20/orm/relationship_api.html#sqlalchemy.orm.relationship)
...
Above, the `Foo.bars` and `Bar.foo` [relationship()](https://docs.sqlalchemy.org/en/20/orm/relationship_api.html#sqlalchemy.orm.relationship) declarations
will raise an error at class construction time because they don’t use
[Mapped](https://docs.sqlalchemy.org/en/20/orm/internals.html#sqlalchemy.orm.Mapped) (by contrast, the annotations that use
[Column](https://docs.sqlalchemy.org/en/20/core/metadata.html#sqlalchemy.schema.Column) are ignored by 2.0, as these are able to be
recognized as a legacy configuration style). To allow all annotations that
don’t use [Mapped](https://docs.sqlalchemy.org/en/20/orm/internals.html#sqlalchemy.orm.Mapped) to pass without error,
...
The `__allow_unmapped__` directive applies **only** to the
...
at [Migrating an Existing Mapping](https://docs.sqlalchemy.org/en/20/changelog/whatsnew_20.html#whatsnew-20-orm-typing-migration).
...
that don’t use [Mapped](https://docs.sqlalchemy.org/en/20/orm/internals.html#sqlalchemy.orm.Mapped) to remain usable.
...
The guide at [What’s New in SQLAlchemy 2.0?](https://docs.sqlalchemy.org/en/20/changelog/whatsnew_20.html) provides an overview of
...
The method that is cross-compatible with [1.x style](https://docs.sqlalchemy.org/en/20/glossary.html#term-1.x-style) and [2.0
style](https://docs.sqlalchemy.org/en/20/glossary.html#term-2.0-style) execution is to make use of the [Connection.begin()](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Connection.begin) method,
or the [Engine.begin()](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Engine.begin) context manager:
...
When using [2.0 style](https://docs.sqlalchemy.org/en/20/glossary.html#term-2.0-style) with the [create_engine.future](https://docs.sqlalchemy.org/en/20/core/engines.html#sqlalchemy.create_engine.params.future)
flag, “commit as you go” style may also be used, as the
[Connection](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Connection) features **autobegin** behavior, which takes place
when a statement is first invoked in the absence of an explicit call to
[Connection.begin()](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Connection.begin):
...
When [2.0 deprecations mode](#migration-20-deprecations-mode) is enabled,
...
([PEP 249](https://peps.python.org/pep-0249/)) in that it tried to hide [PEP 249](https://peps.python.org/pep-0249/)’s emphasis on “implicit begin”
...
[Session](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session) is normally used today, the “future” version of
[Connection](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Connection), which is the one that is returned from an
[Engine](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Engine) that was created using the
[create_engine.future](https://docs.sqlalchemy.org/en/20/core/engines.html#sqlalchemy.create_engine.params.future) flag, includes new
...
Above, the `engine.connect()` method will return a [Connection](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Connection) that
...
is featured both by [Connection](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Connection) as well as the ORM
[Session](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session) object; autobegin allows that the [Connection.begin()](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Connection.begin)
...
“connectionless” execution discussed at [“Implicit” and “Connectionless” execution, “bound metadata” removed](#migration-20-implicit-execution).
...
[Connection.execution_options.isolation_level](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Connection.execution_options.params.isolation_level) parameter as
discussed at [Setting Transaction Isolation Levels including DBAPI Autocommit](https://docs.sqlalchemy.org/en/20/core/connections.html#dbapi-autocommit).  True autocommit is treated as an “isolation level”
so that the structure of application code does not change when autocommit is
used; the [Connection.begin()](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Connection.begin) context manager as well as
methods like [Connection.commit()](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Connection.commit) may still be used, they are
...
The ability to associate an [Engine](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Engine) with a [MetaData](https://docs.sqlalchemy.org/en/20/core/metadata.html#sqlalchemy.schema.MetaData)
...
For schema level patterns, explicit use of an [Engine](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Engine)
or [Connection](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Connection) is required.   The [Engine](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Engine)
may still be used directly as the source of connectivity for a
[MetaData.create_all()](https://docs.sqlalchemy.org/en/20/core/metadata.html#sqlalchemy.schema.MetaData.create_all) operation or autoload operation.
For executing statements, only the [Connection](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Connection) object
has a [Connection.execute()](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Connection.execute) method (in addition to
the ORM-level [Session.execute()](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session.execute) method):
...
invoking `.execute()` from the [Engine](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Engine):

```
result = engine.execute(some_statement)
```

The above operation implicitly procures a [Connection](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Connection) object,
...
- The feature relies upon the “application level autocommit” feature in order
  ...
  [inefficient and misleading](#migration-20-autocommit).
...
the [Connection](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Connection) object was introduced and later Python
...
be somewhat convenient for working within [MetaData.create_all()](https://docs.sqlalchemy.org/en/20/core/metadata.html#sqlalchemy.schema.MetaData.create_all)
calls as well as with [Session](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session) objects, however having these
functions receive an [Engine](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Engine) explicitly provides for clearer
application design.

#### Many Choices becomes One Choice

Overall, the above executional patterns were introduced in SQLAlchemy’s
very first 0.1 release before the [Connection](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Connection) object even existed.
...
[Connection](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Connection) and then to explicitly demarcate the transaction,
...
The argument patterns that may be used with the [sqlalchemy.engine.Connection()](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Connection)
...
[sqlalchemy.engine.Connection()](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Connection). The examples below illustrate the patterns that
...
The new [Connection.execute()](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Connection.execute) method now accepts a subset of the
argument styles that are accepted by the 1.x [Connection.execute()](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Connection.execute)
...
[Connection.execute.execution_options](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Connection.execute.params.execution_options) dictionary that is now
...
[Session.execute()](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session.execute) method, which is a much more prominent API in 2.0
style.

The removal of direct string SQL is to resolve an inconsistency between
[Connection.execute()](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Connection.execute) and [Session.execute()](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session.execute),
where in the former case the string is passed to the driver raw, and in the
latter case it is first converted to a [text()](https://docs.sqlalchemy.org/en/20/core/sqlelement.html#sqlalchemy.sql.expression.text) construct.  By
allowing only [text()](https://docs.sqlalchemy.org/en/20/core/sqlelement.html#sqlalchemy.sql.expression.text) this also limits the accepted parameter
...
the [text()](https://docs.sqlalchemy.org/en/20/core/sqlelement.html#sqlalchemy.sql.expression.text) construct has come to represent an explicit boundary
...
Version 1.4 introduces an [all new Result object](https://docs.sqlalchemy.org/en/20/changelog/migration_14.html#change-result-14-core)
that in turn returns [Row](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Row) objects, which behave like named
...
selecting rows from the [Query](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query) object has been replaced by the
[Row](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Row) class, which is the base of the same [Row](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Row) that comes
back with Core statement results when using the
[create_engine.future](https://docs.sqlalchemy.org/en/20/core/engines.html#sqlalchemy.create_engine.params.future) flag with [Engine](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Engine) (when
the [create_engine.future](https://docs.sqlalchemy.org/en/20/core/engines.html#sqlalchemy.create_engine.params.future) flag is not set, Core result sets use
...
[Row](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Row) class directly).

This [Row](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Row) behaves like a named tuple, in that it acts as a sequence
...
The [Row](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Row) class as used by the ORM also supports access via entity
or attribute:
...
The [select()](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.select) construct as well as the related method [FromClause.select()](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.FromClause.select)
...
sent positionally, rather than as a list.  Additionally, the [case()](https://docs.sqlalchemy.org/en/20/core/sqlelement.html#sqlalchemy.sql.expression.case) construct
...
Only the “generative” style of [select()](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.select) will be supported.  The list
of columns / tables to SELECT from should be passed positionally.  The
[select()](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.select) construct in SQLAlchemy 1.4 accepts both the legacy
...
should be passed **as lists**.   For many years, the [select()](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.select)
...
SQLAlchemy 2.0 finally resolves this by changing the [select()](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.select) construct
...
In a similar way as to the previous change to [select()](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.select), the
constructor arguments to [insert()](https://docs.sqlalchemy.org/en/20/core/dml.html#sqlalchemy.sql.expression.insert), [update()](https://docs.sqlalchemy.org/en/20/core/dml.html#sqlalchemy.sql.expression.update) and
[delete()](https://docs.sqlalchemy.org/en/20/core/dml.html#sqlalchemy.sql.expression.delete) other than the table argument are essentially removed:
...
manner as that of the [select()](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.select) construct.
...
moved to the `sqlalchemy.orm` package.  The [declarative_base()](https://docs.sqlalchemy.org/en/20/orm/mapping_api.html#sqlalchemy.orm.declarative_base)
and [declared_attr()](https://docs.sqlalchemy.org/en/20/orm/mapping_api.html#sqlalchemy.orm.declared_attr) functions are present without any behavioral
changes.  A new super-implementation of [declarative_base()](https://docs.sqlalchemy.org/en/20/orm/mapping_api.html#sqlalchemy.orm.declarative_base)
known as [registry](https://docs.sqlalchemy.org/en/20/orm/mapping_api.html#sqlalchemy.orm.registry) now serves as the top-level ORM configurational
...
at [Declarative is now integrated into the ORM with new features](https://docs.sqlalchemy.org/en/20/changelog/migration_14.html#change-5508).

See also

[ORM Mapped Class Overview](https://docs.sqlalchemy.org/en/20/orm/mapping_styles.html) - all new unified documentation for
...
[registry.map_imperatively()](https://docs.sqlalchemy.org/en/20/orm/mapping_api.html#sqlalchemy.orm.registry.map_imperatively) taken from a [registry](https://docs.sqlalchemy.org/en/20/orm/mapping_api.html#sqlalchemy.orm.registry)
object.
...
To work from a central [registry](https://docs.sqlalchemy.org/en/20/orm/mapping_api.html#sqlalchemy.orm.registry) object:
...
The above [registry](https://docs.sqlalchemy.org/en/20/orm/mapping_api.html#sqlalchemy.orm.registry) is also the source for declarative mappings,
and classical mappings now have access to this registry including string-based
configuration on [relationship()](https://docs.sqlalchemy.org/en/20/orm/relationship_api.html#sqlalchemy.orm.relationship):
...
form of it is based off of the [registry](https://docs.sqlalchemy.org/en/20/orm/mapping_api.html#sqlalchemy.orm.registry) object and is available
as [registry.map_imperatively()](https://docs.sqlalchemy.org/en/20/orm/mapping_api.html#sqlalchemy.orm.registry.map_imperatively).

In addition, the primary rationale used for “classical mapping” is that of
keeping the [Table](https://docs.sqlalchemy.org/en/20/core/metadata.html#sqlalchemy.schema.Table) setup distinct from the class.  Declarative
has always allowed this style using so-called
[hybrid declarative](https://docs.sqlalchemy.org/en/20/orm/declarative_tables.html#orm-imperative-table-configuration). However, to
remove the base class requirement, a first class [decorator](https://docs.sqlalchemy.org/en/20/orm/declarative_mapping.html) form has been added.

As yet another separate but related enhancement, support for [Python
dataclasses](https://docs.sqlalchemy.org/en/20/orm/dataclasses.html#orm-declarative-dataclasses) is added as well to both
...
[Session.execute()](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session.execute) in conjunction with [select()](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.select) to run ORM
queries, instead of using [Session.query()](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session.query).  As mentioned elsewhere,
there is no plan to actually remove the [Session.query()](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session.query) API itself,
...
| 1.x styleform | 2.0 styleform | See Also |
| --- | --- | --- |
| session.query(User).get(42) | session.get(User,42) | ORM Query - get() method moves to Session |
| session.query(User).all() | session.execute(select(User)).scalars().all()# orsession.scalars(select(User)).all() | ORM Query Unified with Core SelectSession.scalars()Result.scalars() |
| session.query(User).\filter_by(name="some user").\one() | session.execute(select(User).filter_by(name="some user")).scalar_one() | ORM Query Unified with Core SelectResult.scalar_one() |
| session.query(User).\filter_by(name="some user").\first() | session.scalars(select(User).filter_by(name="some user").limit(1)).first() | ORM Query Unified with Core SelectResult.first() |
| session.query(User).options(joinedload(User.addresses)).all() | session.scalars(select(User).options(joinedload(User.addresses))).unique().all() | ORM Rows not uniquified by default |
| session.query(User).\join(Address).\filter(Address.email=="[email protected]").\all() | session.execute(select(User).join(Address).where(Address.email=="[email protected]")).scalars().all() | ORM Query Unified with Core SelectJoins |
| session.query(User).\from_statement(text("select * from users")).\all() | session.scalars(select(User).from_statement(text("select * from users"))).all() | Getting ORM Results from Textual Statements |
| session.query(User).\join(User.addresses).\options(contains_eager(User.addresses)).\populate_existing().all() | session.execute(select(User).join(User.addresses).options(contains_eager(User.addresses)).execution_options(populate_existing=True)).scalars().all() | ORM Execution OptionsPopulate Existing |
| session.query(User).\filter(User.name=="foo").\update({"fullname":"Foo Bar"},synchronize_session="evaluate") | session.execute(update(User).where(User.name=="foo").values(fullname="Foo Bar").execution_options(synchronize_session="evaluate")) | ORM-Enabled INSERT, UPDATE, and DELETE statements |
| session.query(User).count() | session.scalar(select(func.count()).select_from(User))# orsession.scalar(select(func.count(User.id))) | Session.scalar() |

### ORM Query Unified with Core Select

**Synopsis**

The [Query](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query) object (as well as the [BakedQuery](https://docs.sqlalchemy.org/en/20/orm/extensions/baked.html#sqlalchemy.ext.baked.BakedQuery) and
[ShardedQuery](https://docs.sqlalchemy.org/en/20/orm/extensions/horizontal_shard.html#sqlalchemy.ext.horizontal_shard.ShardedQuery) extensions) become long term legacy objects,
replaced by the direct usage of the [select()](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.select) construct in conjunction
with the [Session.execute()](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session.execute) method.  Results
that are returned from [Query](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query) in the form of lists of objects
or tuples, or as scalar ORM objects are returned from [Session.execute()](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session.execute)
uniformly as [Result](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Result) objects, which feature an interface
...
# becomes legacy use case
for user in (
    session.query(User).join(User.addresses).filter(Address.email == "[email protected]")
):
    ...
...
[Query](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query) objects as well as that the [Query](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query) interface
...
supported for the most part.  The [select()](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.select) construct now suits
both the Core and ORM use cases, which when invoked via the [Session.execute()](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session.execute)
method will return ORM-oriented results, that is, ORM objects if that’s what
was requested.

The [Select()](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.Select) construct **adds many new methods** for
compatibility with [Query](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query), including [Select.filter()](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.Select.filter) [Select.filter_by()](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.Select.filter_by), newly reworked [Select.join()](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.Select.join)
and [Select.outerjoin()](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.Select.outerjoin) methods, [Select.options()](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.Select.options),
etc.    Other more supplemental methods of [Query](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query) such as
[Query.populate_existing()](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query.populate_existing) are implemented via execution options.

Return results are in terms of a
[Result](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Result) object, the new version of the SQLAlchemy
`ResultProxy` object, which also adds many new methods for compatibility
with [Query](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query), including [Result.one()](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Result.one), [Result.all()](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Result.all),
[Result.first()](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Result.first), [Result.one_or_none()](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Result.one_or_none), etc.

The [Result](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Result) object however does require some different calling
...
single ORM objects the way [Query](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query) does, the [Result.scalars()](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Result.scalars)
modifier must be called first.  In order to return uniqued objects, as is
necessary when using joined eager loading, the [Result.unique()](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Result.unique)
modifier must be called first.

Documentation for all new features of [select()](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.select) including execution
options, etc. are at [ORM Querying Guide](https://docs.sqlalchemy.org/en/20/orm/queryguide/index.html).

Below are some examples of how to migrate to [select()](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.select):
...
for user in session.execute(
    select(User).join(User.addresses).filter(Address.email == "[email protected]")
).scalars():
    ...
...
The fact that SQLAlchemy has both a [select()](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.select) construct
as well as a separate [Query](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query) object that features an extremely
...
In SQLAlchemy’s first releases, the [Query](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query) object didn’t exist
at all.  The original idea was that the [Mapper](https://docs.sqlalchemy.org/en/20/orm/mapping_api.html#sqlalchemy.orm.Mapper) construct itself would
be able to select rows, and that [Table](https://docs.sqlalchemy.org/en/20/core/metadata.html#sqlalchemy.schema.Table) objects, not classes,
would be used to create the various criteria in a Core-style approach.   The
[Query](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query) came along some months / years into SQLAlchemy’s history
...
were not present in SQLAlchemy previously, and the [select()](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.select) construct
used only the “all-at-once” construction style that’s now deprecated
at [select() no longer accepts varied constructor arguments, columns are passed positionally](#migration-20-5284).

As the new approach took off, the object evolved into the [Query](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query)
...
from a [Query](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query) object rather than from a [select](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.select)
object were added.   The goal became that [Query](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query) should have the
full functionality of [select](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.select) in that it could be composed to
build SELECT statements fully with no explicit use of [select()](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.select)
needed.   At the same time, [select()](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.select) had also evolved “generative”
methods like [Select.where()](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.Select.where) and [Select.order_by()](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.Select.order_by).
...
objects was that the [select()](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.select) object needed to remain **completely
agnostic of the ORM**.  To achieve this, the vast majority of logic from
[Query](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query) has been moved into the SQL compile phase, where
ORM-specific compiler plugins receive the
[Select](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.Select) construct and interpret its contents in terms of an
ORM-style query, before passing off to the core-level compiler in order to
create a SQL string.  With the advent of the new
[SQL compilation caching system](https://docs.sqlalchemy.org/en/20/changelog/migration_14.html#change-4639),
...
The [Query.get()](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query.get) method remains for legacy purposes, but the
primary interface is now the [Session.get()](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session.get) method:
...
In 1.4 / 2.0, the [Session](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session) object adds a new
[Session.get()](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session.get) method:
...
The [Query](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query) object is to be a legacy object in 2.0, as ORM
queries are now available using the [select()](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.select) object.  As the
[Query.get()](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query.get) method defines a special interaction with the
[Session](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session) and does not necessarily even emit a query, it’s more
appropriate that it be part of [Session](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session), where it is similar
to other “identity” methods such as [refresh](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session.refresh) and
[merge](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session.merge).
...
wrong as this method is really more about the [Session](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session) than
...
This refers to patterns such as that of [Query.join()](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query.join) as well as
query options like [joinedload()](https://docs.sqlalchemy.org/en/20/orm/queryguide/relationships.html#sqlalchemy.orm.joinedload) which currently accept a mixture of
...
The same techniques apply to [2.0-style](https://docs.sqlalchemy.org/en/20/glossary.html#term-1) style use:
...
Use individual calls to [Query.join()](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query.join) for 1.x /2.0 cross compatible
use:
...
For [2.0-style](https://docs.sqlalchemy.org/en/20/glossary.html#term-1) use, [Select](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.Select) has the same behavior of
[Select.join()](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.Select.join), and also features a new [Select.join_from()](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.Select.join_from)
...
interface of methods such as [Select.join()](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.Select.join).

### ORM Query - join(…, aliased=True), from_joinpoint removed

**Synopsis**

The `aliased=True` option on [Query.join()](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query.join) is removed, as is
...
The `aliased=True` option on [Query.join()](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query.join) is another feature that
...
when the [Query](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query) object didn’t yet have good capabilities regarding
joining along relationship attributes, functions like
[PropComparator.of_type()](https://docs.sqlalchemy.org/en/20/orm/internals.html#sqlalchemy.orm.PropComparator.of_type) did not exist, and the [aliased()](https://docs.sqlalchemy.org/en/20/orm/queryguide/api.html#sqlalchemy.orm.aliased)
...
[Query](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query) will automatically add columns in the ORDER BY when
...
[Result.columns()](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Result.columns) method:
...
This case is an example of the limited flexibility of [Query](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query)
...
The `Query.from_self()` method will be removed from [Query](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query):
...
The [aliased()](https://docs.sqlalchemy.org/en/20/orm/queryguide/api.html#sqlalchemy.orm.aliased) construct may be used to emit ORM queries against
...
used in [1.x style](https://docs.sqlalchemy.org/en/20/glossary.html#term-1.x-style) with [Query](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query) as below; note that
since the final query wants to query in terms of both the `User` and
`Address` entities, two separate [aliased()](https://docs.sqlalchemy.org/en/20/orm/queryguide/api.html#sqlalchemy.orm.aliased) constructs are created:
...
The same form may be used in [2.0 style](https://docs.sqlalchemy.org/en/20/glossary.html#term-2.0-style):
...
The `Query.from_self()` method is a very complicated method that is rarely
used.   The purpose of this method is to convert a [Query](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query) into a
subquery, then return a new [Query](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query) which SELECTs from that subquery.
...
Because `Query.from_self()` packs an intense amount of implicit
...
The new approach makes use of the [aliased()](https://docs.sqlalchemy.org/en/20/orm/queryguide/api.html#sqlalchemy.orm.aliased) construct so that the
...
As is the case described at [Selecting from the query itself as a subquery, e.g. “from_self()”](#migration-20-query-from-self), the
[aliased()](https://docs.sqlalchemy.org/en/20/orm/queryguide/api.html#sqlalchemy.orm.aliased) object provides a single place that operations like
“select entity from a subquery” may be achieved.  Using [1.x style](https://docs.sqlalchemy.org/en/20/glossary.html#term-1.x-style):
...
Using [2.0 style](https://docs.sqlalchemy.org/en/20/glossary.html#term-2.0-style):
...
[Selecting from the query itself as a subquery, e.g. “from_self()”](#migration-20-query-from-self).   The `Query.select_from_entity()`
...
`Query.from_self()`, it’s much easier to follow what’s going on
when using an explicit [aliased()](https://docs.sqlalchemy.org/en/20/orm/queryguide/api.html#sqlalchemy.orm.aliased) object, both from a user point
...
[Result.unique()](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Result.unique) method is called.  The ORM will actually set
...
[Result.unique()](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Result.unique) method, such that they will not be confused when
...
To the degree that having to call [Result.unique()](https://docs.sqlalchemy.org/en/20/core/connections.html#sqlalchemy.engine.Result.unique) when joined
eager load collections are used is inconvenient, in modern SQLAlchemy
the [selectinload()](https://docs.sqlalchemy.org/en/20/orm/queryguide/relationships.html#sqlalchemy.orm.selectinload) strategy presents a collection-oriented
eager loader that is superior in most respects to [joinedload()](https://docs.sqlalchemy.org/en/20/orm/queryguide/relationships.html#sqlalchemy.orm.joinedload)
...
[Dynamic Relationship Loaders](https://docs.sqlalchemy.org/en/20/orm/large_collections.html#dynamic-relationship), makes use of the [Query](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query) object
...
through the [WriteOnlyCollection](https://docs.sqlalchemy.org/en/20/orm/large_collections.html#sqlalchemy.orm.WriteOnlyCollection) collection class
...
direct integrations with the new [ORM-enabled Bulk DML](https://docs.sqlalchemy.org/en/20/changelog/whatsnew_20.html#change-8360)
featureset.
...
[AppenderQuery](https://docs.sqlalchemy.org/en/20/orm/large_collections.html#sqlalchemy.orm.AppenderQuery), which is the collection type used by the “dynamic”
strategy, to [WriteOnlyCollection](https://docs.sqlalchemy.org/en/20/orm/large_collections.html#sqlalchemy.orm.WriteOnlyCollection), which is the collection type
...
- Make use of the [Query.statement](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query.statement) attribute on an existing
  `lazy="dynamic"` relationship.   We can use methods like
  [Session.scalars()](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session.scalars) with the dynamic loader straight away as
  follows:
  ```
  class User(Base):
      __tablename__ = "user"
  ...
  ```
- Use the [with_parent()](https://docs.sqlalchemy.org/en/20/orm/queryguide/api.html#sqlalchemy.orm.with_parent) function to construct a [select()](https://docs.sqlalchemy.org/en/20/core/selectable.html#sqlalchemy.sql.expression.select)
  construct directly:
  ```
...
The original idea was that the [with_parent()](https://docs.sqlalchemy.org/en/20/orm/queryguide/api.html#sqlalchemy.orm.with_parent) function should be
...
new feature is as [New “Write Only” relationship strategy supersedes “dynamic”](https://docs.sqlalchemy.org/en/20/changelog/whatsnew_20.html#change-7123).

See also

[New “Write Only” relationship strategy supersedes “dynamic”](https://docs.sqlalchemy.org/en/20/changelog/whatsnew_20.html#change-7123)
...
The [Session](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session) will no longer support “autocommit” mode, that
is, this pattern:
...
The main reason a [Session](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session) is used in “autocommit” mode
is so that the [Session.begin()](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session.begin) method is available, so that framework
integrations and event hooks can control when this event happens.  In 1.4,
the [Session](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session) now features [autobegin behavior](https://docs.sqlalchemy.org/en/20/changelog/migration_14.html#change-5074)
which resolves this issue; the [Session.begin()](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session.begin) method may now
be called:
...
explicit use of [Session.begin()](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session.begin), which is now solved by 1.4,
...
[Session.begin()](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session.begin) method when a transaction were already begun,
resulting in a construct called a “subtransaction”, which was essentially
a block that would prevent the [Session.commit()](https://docs.sqlalchemy.org/en/20/orm/session_api.html#sqlalchemy.orm.Session.commit) method from actually
committing.
...
As the [Query](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query) object becomes legacy, these two recipes
which previously relied upon subclassing of the [Query](https://docs.sqlalchemy.org/en/20/orm/queryguide/query.html#sqlalchemy.orm.Query)
object now make use of the [SessionEvents.do_orm_execute()](https://docs.sqlalchemy.org/en/20/orm/events.html#sqlalchemy.orm.SessionEvents.do_orm_execute)
hook.    See the section [Re-Executing Statements](https://docs.sqlalchemy.org/en/20/orm/session_events.html#do-orm-execute-re-executing) for
an example.
...
See [SQL Compilation Caching](https://docs.sqlalchemy.org/en/20/core/connections.html#sql-caching) for full background on the new caching system.
...
See [Asynchronous IO Support for Core and ORM](https://docs.sqlalchemy.org/en/20/changelog/migration_14.html#change-3414) for background.
