...
A `+page.svelte` component defines a page of your app. By default, pages are rendered both on the server ([SSR](https://kit.svelte.dev/docs/glossary#SSR)) for the initial request and in the browser ([CSR](https://kit.svelte.dev/docs/glossary#CSR)) for subsequent navigation.
...
This function runs alongside `+page.svelte`, which means it runs on the server during server-side rendering and in the browser during client-side navigation. See [load](https://kit.svelte.dev/docs/load) for full details of the API.
...
You can find more information about these in [page options](https://kit.svelte.dev/docs/page-options).

### +page.server.js

If your `load` function can only run on the server — for example, if it needs to fetch data from a database or you need to access private [environment variables](https://kit.svelte.dev/docs/$env-static-private) like API keys — then you can rename `+page.js` to `+page.server.js` and change the `PageLoad` type to `PageServerLoad`.
...
During client-side navigation, SvelteKit will load this data from the server, which means that the returned value must be serializable using [devalue](https://github.com/rich-harris/devalue). See [load](https://kit.svelte.dev/docs/load) for full details of the API.

Like `+page.js`, `+page.server.js` can export [page options](https://kit.svelte.dev/docs/page-options) — `prerender`, `ssr` and `csr`.

A `+page.server.js` file can also export *actions*. If `load` lets you read data from the server, `actions` let you write data *to* the server using the `<form>` element. To learn how to use them, see the [form actions](https://kit.svelte.dev/docs/form-actions) section.
...
> `+error.svelte` is *not* used when an error occurs inside [handle](https://kit.svelte.dev/docs/hooks#Server-hooks-handle) or a [+server.js](#server) request handler.

You can read more about error handling [here](https://kit.svelte.dev/docs/errors).
...
By default, each layout inherits the layout above it. Sometimes that isn’t what you want - in this case, [advanced layouts](https://kit.svelte.dev/docs/advanced-routing#Advanced-layouts) can help you.
...
> Often, layout data is unchanged when navigating between pages. SvelteKit will intelligently rerun [load](https://kit.svelte.dev/docs/load) functions when necessary.

### +layout.server.js

To run your layout’s `load` function on the server, move it to `+layout.server.js`, and change the `LayoutLoad` type to `LayoutServerLoad`.

Like `+layout.js`, `+layout.server.js` can export [page options](https://kit.svelte.dev/docs/page-options) — `prerender`, `ssr` and `csr`.

## +server

As well as pages, you can define routes with a `+server.js` file (sometimes referred to as an ‘API route’ or an ‘endpoint’), which gives you full control over the response. Your `+server.js` file exports functions corresponding to HTTP verbs like `GET`, `POST`, `PATCH`, `PUT`, `DELETE`, `OPTIONS`, and `HEAD` that take a [RequestEvent](https://kit.svelte.dev/docs/@sveltejs-kit#RequestEvent) argument and return a [Response](https://developer.mozilla.org/en-US/docs/Web/API/Response) object.
...
You can use the [error](https://kit.svelte.dev/docs/@sveltejs-kit#error), [redirect](https://kit.svelte.dev/docs/@sveltejs-kit#redirect) and [json](https://kit.svelte.dev/docs/@sveltejs-kit#json) methods from `@sveltejs/kit` for convenience (but you don’t have to).

If an error is thrown (either `error(...)` or an unexpected error), the response will be a JSON representation of the error or a fallback error page — which can be customised via `src/error.html` — depending on the `Accept` header. The [+error.svelte](#error) component will *not* be rendered in this case. You can read more about error handling [here](https://kit.svelte.dev/docs/errors).
...
> `+layout` files have no effect on `+server.js` files. If you want to run some logic before each request, add it to the server [handle](https://kit.svelte.dev/docs/hooks#Server-hooks-handle) hook.
...
> In general, [form actions](https://kit.svelte.dev/docs/form-actions) are a better way to submit data from the browser to the server.
...
You can read more about omitting `$types` in our [blog post](https://kit.svelte.dev/blog/zero-config-type-safety) about it.

## Other files

Any other files inside a route directory are ignored by SvelteKit. This means you can colocate components and utility modules with the routes that need them.

If components and modules are needed by multiple routes, it’s a good idea to put them in [$lib](https://kit.svelte.dev/docs/$lib).

## Further reading

- [Tutorial: Routing](https://kit.svelte.dev/tutorial/kit/pages)
- [Tutorial: API routes](https://kit.svelte.dev/tutorial/kit/get-handlers)
- [Docs: Advanced routing](https://kit.svelte.dev/docs/advanced-routing)

[Edit this page on GitHub](https://github.com/sveltejs/kit/edit/main/documentation/docs/20-core-concepts/10-routing.md) [[llms.txt](https://kit.svelte.dev/docs/kit/routing/llms.txt)]

 previous next [[Web standards](https://kit.svelte.dev/docs/kit/web-standards)] [[Loading data](https://kit.svelte.dev/docs/kit/load)]
