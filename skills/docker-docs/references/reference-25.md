...
The options that you can pass to `--tmpfs` are identical to the Linux `mount -t tmpfs -o` command. The following example mounts an empty `tmpfs` into the
...
binary (refer to
[get the Linux binary](https://docs.docker.com/engine/install/binaries/#install-static-binaries)),
...
For in-depth information about volumes, refer to
...
host. You can also specify `udp` and `sctp` ports. The
[Networking overview
...
> Note
>
> If you don't specify an IP address (i.e., `-p 80:80` instead of `-p 127.0.0.1:80:80`) when publishing a container's ports, Docker publishes the
...
fails the detached container paradigm in that, the root process (`service nginx start`) returns and the detached container stops as designed. As a result, the
...
- The CDI feature has been enabled in the daemon; see
  [Enable CDI
...
See also
[thedocker cpcommand](https://docs.docker.com/reference/cli/docker/container/cp/).
...
Read
[Specify a container's resources](https://docs.docker.com/config/containers/resource_constraints/)
...
`Restarting` in
[docker ps](https://docs.docker.com/reference/cli/docker/container/ls/). It can also be useful to use
...
| Value | Description |
| --- | --- |
| default | Use the value specified by the Docker daemon's--exec-optor system default (see below). |
