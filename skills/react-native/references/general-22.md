...
We recommend that you only use small snapshots (see [no-large-snapshotsrule](https://github.com/jest-community/eslint-plugin-jest/blob/main/docs/rules/no-large-snapshots.md)). If you want to test a *change* between two React component states, use [snapshot-diff](https://github.com/jest-community/snapshot-diff). When in doubt, prefer explicit expectations as described in the previous paragraph.
