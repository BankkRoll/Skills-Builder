...
def get_scraper(platform: str = "auto"):
    ...
    if not scraper_class:
        available = [k for k in SCRAPERS.keys() if k != "auto"]
...
def detect_platform(url: str) -> str:
    ...
    Only matches official platform domains. For custom domains,
    use -p/--platform to specify the platform explicitly.
    """
    url_lower = url.lower()

    # ReadTheDocs - official hosting domains
    if ".readthedocs.io" in url_lower or ".readthedocs.org" in url_lower:
        return "readthedocs"

    # GitBook - official hosting domain
    if ".gitbook.io" in url_lower:
        return "gitbook"

    # Docusaurus - the framework's own docs
    if "docusaurus.io" in url_lower:
        return "docusaurus"

    # Mintlify - official domains
    if "mintlify.com" in url_lower or "mintlify.dev" in url_lower:
        return "mintlify"

    # Default to mintlify for unknown sites (handles JS rendering well)
    return "mintlify"
